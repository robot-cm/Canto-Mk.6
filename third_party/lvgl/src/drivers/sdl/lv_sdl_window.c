...
/* Round 4 diagnostic: lock the uploaded texture and copy its pixels out so a
 * headless test can inspect EXACTLY what SDL is presenting (bypasses the LVGL
 * internal draw buffer, which would not reveal a driver/format/pitch bug).
 * Returns pitch (bytes/row) on success, <0 on failure. */
int lv_sdl_window_read_texture(lv_display_t * disp, uint8_t * out, uint32_t out_size)
{
    lv_sdl_window_t * dsc = lv_display_get_driver_data(disp);
    if(!dsc || !dsc->texture) return -1;
    void * pixels = NULL;
    int pitch = 0;
    if(SDL_LockTexture(dsc->texture, NULL, &pixels, &pitch) != 0) return -2;
    uint32_t need = (uint32_t)pitch * (uint32_t)disp->ver_res;
    if(out_size < need) { SDL_UnlockTexture(dsc->texture); return -3; }
    memcpy(out, pixels, need);
    SDL_UnlockTexture(dsc->texture);
    return pitch;
}
...
static void window_create(lv_display_t * disp)
{
    ...
    /* Round 4 fix: initialize the framebuffer to BLACK (0x0000), not white
     * (0xffff). If any row is ever left unrendered it would otherwise stay
     * white and bleed through as the "white bottom" the user reported. */
...
static void window_update(lv_display_t * disp)
{
    ...
    /* Round 4 fix: SDL_RenderClear() defaults to WHITE. On a real window the
     * 240x240 texture may not cover the full renderer output (HiDPI scaling,
     * window resize, or a non-square window), so the uncovered band showed as
     * a glaring white letterbox — exactly the "top black / bottom white"
     * simulator split the user reported. Clear to BLACK (the OS background)
     * instead, and copy to the FULL renderer-output rect so the content always
     * covers. */
    SDL_SetRenderDrawColor(dsc->renderer, 0, 0, 0, 255);
    SDL_RenderClear(dsc->renderer);

    /* Letterbox the texture: preserve the square (1:1) aspect ratio of the
     * round watch and center it with black bars. This keeps the circular
     * display undistorted on ANY window size (user-resized, HiDPI-scaled, or
     * non-square), instead of a stretched frame whose top/bottom bands look
     * like the reported "top white / bottom black" split. */
    int out_w = 0, out_h = 0;
    SDL_GetRendererOutputSize(dsc->renderer, &out_w, &out_h);
    int side = (out_w < out_h) ? out_w : out_h;
    SDL_Rect dst = { (out_w - side) / 2, (out_h - side) / 2, side, side };
    SDL_RenderCopy(dsc->renderer, dsc->texture, NULL, &dst);
#endif
...
#error("Unsupported color format")
    ...
    /* RGB565/RGB888/BGR24 have NO alpha channel. Leaving SDL_BLENDMODE_BLEND on
     * such a texture is pointless and on several Windows GPU drivers makes
     * SDL_RenderCopy blend the frame as if it were semi-transparent — exactly
     * the "half-sheet of glass / top white bottom black" artifact the user
     * sees. Only the I1->ARGB8888 path carries real alpha, so use NONE for
     * every opaque format. */
#if LV_COLOR_DEPTH == 1
