...
void indev_gesture(lv_indev_t * indev)
{
    ...
    if((LV_ABS(indev->pointer.gesture_sum.x) > indev_act->gesture_limit) ||
       (LV_ABS(indev->pointer.gesture_sum.y) > indev_act->gesture_limit)) {
        ...
        /* EOS fix: pass the indev as the event param (not the gesture object).
         * lv_event_get_indev() reads the param for GESTURE events, so object
         * callbacks got the wrong pointer and indev-level swipe-back broke. */
        lv_indev_send_event(indev_act, LV_EVENT_GESTURE, indev_act);
        if(indev_reset_check(indev_act)) return;
    }
}
