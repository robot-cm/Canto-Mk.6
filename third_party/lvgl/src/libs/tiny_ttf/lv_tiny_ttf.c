...
static void lv_tiny_ttf_cache_create(ttf_font_desc_t * dsc)
{
    ...
    if(dsc->glyph_cache) {
        ...
        printf("[DEBUG] glyph_cache created max_size=%u\n", (unsigned)dsc->glyph_cache->max_size);
    } else {
        printf("[DEBUG] glyph_cache CREATE FAILED!\n");
    }
    ...
    if(dsc->draw_data_cache) {
        ...
        printf("[DEBUG] draw_data_cache created max_size=%u\n", (unsigned)dsc->draw_data_cache->max_size);
    } else {
        printf("[DEBUG] draw_data_cache CREATE FAILED!\n");
    }
}
