/**
 * @file cos_net_wifi_esp32.h
 * @brief Real Wi-Fi backend for ESP32 (ESP-IDF esp_wifi_*)
 *
 * Platform layer under cos_net_wifi.c (Core System Service). Implements real
 * scanning / connecting via the ESP-IDF Wi-Fi stack. The simulator keeps its
 * own stub backend; this file is only built for COS_PLATFORM_ESP32.
 */

#ifndef COS_NET_WIFI_ESP32_H
#define COS_NET_WIFI_ESP32_H
 ...
 * the netif/event setup, so a later cos_net_wifi_esp32_init() can restart the
 * station without the fragile reinitialization path.
 */
esp_err_t cos_net_wifi_esp32_stop(void);

/**
 * @brief Early-init hook: called by app_main BEFORE cos_init(), while internal
