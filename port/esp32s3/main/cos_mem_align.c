...
#define COS_MEM_ALIGNMENT 8 /* = JMEM_ALIGNMENT (jmem.h: JMEM_ALIGNMENT_LOG 3) */

void *__wrap_malloc(size_t size)
{
    return heap_caps_aligned_alloc(COS_MEM_ALIGNMENT, size, MALLOC_CAP_8BIT);
}
...
void *__wrap_calloc(size_t nmemb, size_t size)
{
    ...
    void *ptr = heap_caps_aligned_alloc(COS_MEM_ALIGNMENT, total, MALLOC_CAP_8BIT);
    if (ptr != NULL)
    {
        memset(ptr, 0, total);
    }
