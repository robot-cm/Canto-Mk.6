/**
 * @file cos_dev_display_gc9a01.h
 * @brief GC9A01 LCD 驱动(CantoMk6 设备 HAL + LVGL 端口)— 板级实现
 *
 * 位置:port/esp32s3/main/(板级专属,非 Core。因引用 esp_lcd.h 等 ESP-IDF 头)
 * Core HAL 接口在 src/devices/display/cos_dev_display.h(平台无关)
 *
 * 实现:
 *   - 基于 ESP-IDF 官方 esp_lcd_panel_gc9a01 组件(AGENTS.md 第28节官方资料优先)
 *   - 包成 CantoMk6 cos_dev_display_ops_t(set_brightness / power_on / power_off)
...
#ifndef COS_DEV_DISPLAY_GC9A01_H
#define COS_DEV_DISPLAY_GC9A01_H
...
/* 注册到 CantoMk6 设备 HAL
 * 把 GC9A01 的 set_brightness/power_on/power_off 注册进 cos_dev_display
 */
...
#endif /* COS_DEV_DISPLAY_GC9A01_H */
