/**
 * @file cos_log.h
 * @brief Listener-based log system
 */

#ifndef COS_LOG_H
#define COS_LOG_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ---------------------------------------------------*/
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>
#include <stdarg.h>
#include "cos_config.h"
#include "cos_error.h"

/* Public macros ----------------------------------------------*/

/************************** Log Listener Flags **************************/
#define COS_LOG_FLAG_SYSTEM (1 << 0) /**< System listener, cannot be deleted */

/************************** Colorful Log **************************/
#if COS_LOG_USE_COLOR
#define COS_LOG_COLOR_RESET "\033[0m"
#define COS_LOG_COLOR_RED "\033[31m"
#define COS_LOG_COLOR_GREEN "\033[32m"
#define COS_LOG_COLOR_YELLOW "\033[33m"
#define COS_LOG_COLOR_BLUE "\033[34m"
#define COS_LOG_COLOR_CYAN "\033[36m"
#else
#define COS_LOG_COLOR_RESET ""
#define COS_LOG_COLOR_RED ""
#define COS_LOG_COLOR_GREEN ""
#define COS_LOG_COLOR_YELLOW ""
#define COS_LOG_COLOR_BLUE ""
#define COS_LOG_COLOR_CYAN ""
#endif /* COS_LOG_USE_COLOR */

/************************** Log Macros **************************/

#ifdef COS_LOG_DISABLE

#define COS_LOG_D(fmt, ...) \
    do                      \
    {                       \
    } while (0)
#define COS_LOG_I(fmt, ...) \
    do                      \
    {                       \
    } while (0)
#define COS_LOG_W(fmt, ...) \
    do                      \
    {                       \
    } while (0)
#define COS_LOG_E(fmt, ...) \
    do                      \
    {                       \
    } while (0)

#else

#ifdef COS_LOG_TAG
#define COS_LOG_FMT(fmt) "[" COS_LOG_TAG "] " fmt
#else
#define COS_LOG_FMT(fmt) fmt
#endif /* COS_LOG_TAG */

#define COS_LOG_D(fmt, ...) cos_log(COS_LOG_LEVEL_DEBUG, COS_LOG_FMT(fmt), ##__VA_ARGS__)
#define COS_LOG_I(fmt, ...) cos_log(COS_LOG_LEVEL_INFO, COS_LOG_FMT(fmt), ##__VA_ARGS__)
#define COS_LOG_W(fmt, ...) cos_log(COS_LOG_LEVEL_WARN, COS_LOG_FMT(fmt), ##__VA_ARGS__)
#define COS_LOG_E(fmt, ...) cos_log(COS_LOG_LEVEL_ERROR, COS_LOG_FMT(fmt), ##__VA_ARGS__)

#endif /* COS_LOG_DISABLE */

/************************** Pointer Check **************************/
#define COS_CHECK_PTR_RETURN(ptr)                                                              \
    do                                                                                         \
    {                                                                                          \
        if (!(ptr))                                                                            \
        {                                                                                      \
            COS_LOG_E("NULL pointer at %s:%d, at function: %s", __FILE__, __LINE__, __func__); \
            return;                                                                            \
        }                                                                                      \
    } while (0)

#define COS_CHECK_PTR_RETURN_FREE(ptr, free_var)                                               \
    do                                                                                         \
    {                                                                                          \
        if (!(ptr))                                                                            \
        {                                                                                      \
            COS_LOG_E("NULL pointer at %s:%d, at function: %s", __FILE__, __LINE__, __func__); \
            cos_free(free_var);                                                                \
            return;                                                                            \
        }                                                                                      \
    } while (0)

#define COS_CHECK_PTR_RETURN_VAL(ptr, ret_val)                                                 \
    do                                                                                         \
    {                                                                                          \
        if (!(ptr))                                                                            \
        {                                                                                      \
            COS_LOG_E("NULL pointer at %s:%d, at function: %s", __FILE__, __LINE__, __func__); \
            return ret_val;                                                                    \
        }                                                                                      \
    } while (0)

#define COS_CHECK_PTR_RETURN_VAL_FREE(ptr, ret_val, free_var)                                  \
    do                                                                                         \
    {                                                                                          \
        if (!(ptr))                                                                            \
        {                                                                                      \
            COS_LOG_E("NULL pointer at %s:%d, at function: %s", __FILE__, __LINE__, __func__); \
            cos_free(free_var);                                                                \
            return ret_val;                                                                    \
        }                                                                                      \
    } while (0)

/************************** Assertion Macros **************************/
#if COS_USE_ASSERT
#define COS_ASSERT(expr)                              \
    do                                                \
    {                                                 \
        if (!(expr))                                  \
        {                                             \
            COS_LOG_E("Assertion failed: %s", #expr); \
            COS_ASSERT_HANDLER                        \
        }                                             \
    } while (0)
#else
#define COS_ASSERT(expr) ((void)0)
#endif /* COS_USE_ASSERT */

/************************** Print Coordinates **************************/

#define COS_DEBUG_PRINT_POS(obj) COS_LOG_D("Obj[%p] (%d,%d)", obj, lv_obj_get_x(obj), lv_obj_get_y(obj))

/* Public typedefs --------------------------------------------*/

typedef int8_t cos_log_listener_id_t;

/**
 * @brief Log level enumeration
 */
typedef enum
{
    COS_LOG_LEVEL_DEBUG = 0, /**< Debug level */
    COS_LOG_LEVEL_INFO, /**< Info level */
    COS_LOG_LEVEL_WARN, /**< Warning level */
    COS_LOG_LEVEL_ERROR, /**< Error level */
    COS_LOG_LEVEL_OFF /**< Silence all log output (runtime Dev-mode switch) */
} cos_log_level_t;

/**
 * @brief Log listener callback function type
 * @param level Log level
 * @param buf Log message buffer
 * @param len Length of log message
 * @param user User data
 */
typedef void (*cos_log_listener_cb_t)(cos_log_level_t level, const char *buf, size_t len, void *user);

/**
 * @brief Log listener structure
 */
typedef struct
{
    const char *name; /**< Listener name (static string) */
    cos_log_listener_cb_t cb; /**< Callback function */
    void *user_data; /**< User data */
    uint8_t used; /**< Whether this slot is used */
    uint8_t flags; /**< Listener flags */
} cos_log_listener_t;

/* Public function prototypes --------------------------------*/

/**
 * @brief Initialize log system
 * @return COS_OK on success
 */
void cos_service_log_init(void);

/**
 * @brief Register a log listener
 * @param name Listener name (must be static string)
 * @param cb Callback function
 * @param user_data User data
 * @param flags Listener flags
 * @return Listener ID (>=0) on success, -1 on failure
 */
cos_log_listener_id_t cos_log_register_listener(const char *name,
                                                cos_log_listener_cb_t cb,
                                                void *user_data,
                                                uint8_t flags);

/**
 * @brief Unregister a log listener by ID
 * @param id Listener ID
 * @return COS_OK on success
 */
cos_result_t cos_log_unregister_listener(cos_log_listener_id_t id);

/**
 * @brief Find a log listener by name
 * @param name Listener name
 * @return Listener ID (>=0) on success, -1 if not found
 */
cos_log_listener_id_t cos_log_find_listener(const char *name);

/**
 * @brief Get listener information by ID
 * @param id Listener ID
 * @param listener Output listener structure
 * @return COS_OK on success
 */
cos_result_t cos_log_get_listener(cos_log_listener_id_t id, cos_log_listener_t *listener);

/**
 * @brief Format and dispatch log message
 * @param level Log level
 * @param fmt Format string
 * @param ... Variable arguments
 */
void cos_log(cos_log_level_t level, const char *fmt, ...);

/**
 * @brief Set the minimum level that will be dispatched.
 * @param level Messages below this level are dropped.
 */
void cos_log_set_min_level(cos_log_level_t level);

/**
 * @brief Get the current minimum dispatched level.
 * @return Current minimum level.
 */
cos_log_level_t cos_log_get_min_level(void);

/**
 * @brief Dispatch log message to all listeners (internal use)
 * @param level Log level
 * @param buf Log message buffer
 * @param len Length of log message
 */
void cos_log_dispatch(cos_log_level_t level, const char *buf, size_t len);

/**
 * @brief Register standard output log listener
 * @return COS_OK on success
 */
cos_result_t cos_service_log_std_register(void);

#ifdef __cplusplus
}
#endif

#endif /* COS_LOG_H */
