/**
 * @file cos_service_power_save.h
 * @brief Power save mode service (省电模式)
 *
 * 省电模式约束:
 *   - 只能显示主界面(watchface),禁止进入其他页面/应用
 *   - 主界面底部按钮长按退出
 *   - 启用时: 低亮度 + 降低 CPU 频率
 */

#ifndef COS_SERVICE_POWER_SAVE_H
#define COS_SERVICE_POWER_SAVE_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ---------------------------------------------------*/
#include <stdbool.h>
#include "cos_core.h"
#include "cos_event.h"

/* Public macros ----------------------------------------------*/

/* 省电模式的熄屏/L2/刷新策略(公共:PM 服务与显示策略据此对齐) */
#define COS_POWER_SAVE_SLEEP_TIMEOUT_SEC 5       /* 无触摸 5s 即熄屏进入 Light Sleep */
#define COS_POWER_SAVE_L2_IDLE_MS        600000  /* 熄屏累计无触摸 10min → L2 自动待机 Deep Sleep */
#define COS_POWER_SAVE_REFR_PERIOD_MS    100     /* LVGL 刷新周期上限(≈10 FPS,限制持续动画刷新) */

/* Public typedefs --------------------------------------------*/

/* Public function prototypes --------------------------------*/

/**
 * @brief Initialize power save service (load persisted state)
 */
cos_result_t cos_service_power_save_init(void);

/**
 * @brief Check whether power save mode is active
 */
bool cos_power_save_is_active(void);

/**
 * @brief Enter power save mode (低亮度 + 降频 + 回主界面 + 锁定)
 */
cos_result_t cos_power_save_enter(void);

/**
 * @brief Exit power save mode (恢复亮度/频率 + 解锁)
 */
cos_result_t cos_power_save_exit(void);

/**
 * @brief Get power-save state change event id (for subscribe)
 */
cos_event_code_t cos_power_save_get_event_id(void);

#ifdef __cplusplus
}
#endif

#endif /* COS_SERVICE_POWER_SAVE_H */
