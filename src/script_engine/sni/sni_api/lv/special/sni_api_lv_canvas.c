...
static void sni_canvas_ctx_delete_cb(lv_event_t *e)
{
    ...
    {
        cos_draw_buf_destroy(ctx->owned_draw_buf);
...
static sni_canvas_buf_ctx_t *sni_canvas_ctx_get_or_create(lv_obj_t *obj)
{
    ...
    sni_canvas_buf_ctx_t *ctx = cos_malloc_zeroed(sizeof(sni_canvas_buf_ctx_t));
    if (!ctx)
    {
        return NULL;
    }
                                            ...
                                            const jerry_length_t args_count)
{
    ...
    lv_draw_buf_t *buf = cos_draw_buf_create((uint32_t)w, (uint32_t)h, cf, stride);
    if (!buf)
    {
