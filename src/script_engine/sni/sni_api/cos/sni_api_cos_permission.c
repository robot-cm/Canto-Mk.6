/**
 * @file sni_api_cos_permission.c
 * @brief SNI permission API implementation
 *
 * cos.permission.request(perm_name, callback)  - async, shows consent panel
 * cos.permission.check(perm_name)              - sync, returns grant state string
 *
...
static bool _queue_has_category(cos_perm_category_t cat);
...
static cos_perm_category_t _resolve_category(const char *name)
{
    cos_perm_category_t cat = cos_permission_name_to_category(name);
    if (cat >= COS_PERM_CATEGORY_COUNT)
    {
        COS_LOG_W("Unknown permission name: %s", name);
    }
    return cat;
}
...
static const char *_state_to_string(cos_perm_state_t state)
{
    switch (state)
    {
        case COS_PERM_STATE_ALLOW_ONCE:
            return "once";
        case COS_PERM_STATE_ALLOW_FOREGROUND:
            return "foreground";
        case COS_PERM_STATE_ALLOW_ALWAYS:
            return "always";
        case COS_PERM_STATE_DENIED:
        default:
            return "denied";
    }
}
...
static void _queue_clear(void)
{
    ...
    while (node)
    {
        ...
        if (node->app_id)
            cos_free(node->app_id);
        if (node->app_name)
            cos_free(node->app_name);
        cos_free(node);
        node = next;
    }
...
static bool _enqueue(cos_perm_category_t cat, const char *app_id, const char *app_name, jerry_value_t callback)
{
    perm_queue_node_t *node = cos_malloc_zeroed(sizeof(perm_queue_node_t));
    if (!node)
        return false;

    node->category = cat;
    node->app_id = cos_strdup(app_id);
    node->app_name = cos_strdup(app_name);
    ...
    COS_LOG_D("Permission request queued: app=%s cat=%d queue_depth>=1", app_id, cat);
    return true;
}
...
static void _show_panel(void)
{
    ...
    cos_perm_panel_t *panel = cos_perm_panel_create(&cfg);
    if (!panel)
    {
        COS_LOG_E("Failed to create permission panel for app=%s cat=%d", _perm_req.app_id, _perm_req.category);
        _cleanup_request();
        _dequeue_and_show();
        return;
    }

    COS_LOG_D("Permission panel shown: app=%s cat=%d", _perm_req.app_id, _perm_req.category);
}
...
static void _dequeue_and_show(void)
{
    ...
    cos_free(node); /* free the node shell, not the transferred fields */
...
static void _schedule_async_granted(jerry_value_t callback, const char *app_id, const char *result_str)
{
    async_granted_ctx_t *ctx = cos_malloc_zeroed(sizeof(async_granted_ctx_t));
    if (!ctx)
        return;
    ...
    cos_dispatcher_call(_async_granted_cb, ctx);
}
...
static void _invoke_callback(jerry_value_t cb, const char *result, const char *app_id)
{
    ...
    {
        COS_LOG_E("Error invoking permission JS callback");
...
static void _cleanup_request(void)
{
    ...
    if (_perm_req.app_id)
    {
        cos_free(_perm_req.app_id);
    }
    if (_perm_req.app_name)
    {
        cos_free(_perm_req.app_name);
    }
                                             ...
                                             const jerry_length_t args_count)
{
    ...
    char *perm_name = cos_malloc(name_len + 1);
    if (!perm_name)
    {
    ...
    cos_perm_category_t cat = _resolve_category(perm_name);
    cos_free(perm_name);
    ...
    cos_perm_state_t current = cos_permission_get(app_id, cat);
    if (current == COS_PERM_STATE_ALLOW_ALWAYS)
    {
        COS_LOG_D("Permission already granted (always) for app %s, category %d", app_id, cat);
        _schedule_async_granted(args_p[1], app_id, "granted_always");
        return jerry_undefined();
    }

    if (current == COS_PERM_STATE_ALLOW_FOREGROUND)
    {
        COS_LOG_D("Permission already granted (foreground) for app %s, category %d", app_id, cat);
    ...
    {
        COS_LOG_D("Permission category %d already pending for app %s, dropping duplicate", cat, app_id);
        return jerry_undefined();
    }
    ...
    _perm_req.app_id = cos_strdup(app_id);
    _perm_req.app_name = cos_strdup(app_name);
                                           ...
                                           const jerry_length_t args_count)
{
    ...
    cos_perm_state_t state = cos_permission_get(app_id, cat);
 ...
 *
 * 1. Persist the user's choice via cos_permission_set()
 ...
 */
static void _perm_button_cb(lv_event_t *e, cos_perm_state_t new_state, const char *result_str)
{
    cos_perm_panel_t *panel = (cos_perm_panel_t *)lv_event_get_user_data(e);

    if (_perm_req.app_id && _perm_req.active)
    {
        if (new_state != COS_PERM_STATE_DENIED)
        {
            cos_permission_set(_perm_req.app_id, _perm_req.category, new_state);
            COS_LOG_D("Permission %s: app=%s cat=%d", result_str, _perm_req.app_id, _perm_req.category);
        }
        else
        {
            COS_LOG_D("Permission denied: app=%s cat=%d", _perm_req.app_id, _perm_req.category);
        }
    }

    _invoke_callback(_perm_req.callback, result_str, _perm_req.app_id);
    _cleanup_request();
    cos_perm_panel_delete(panel);
...
static void _perm_allow_once_cb(lv_event_t *e)
{
    _perm_button_cb(e, COS_PERM_STATE_ALLOW_ONCE, "granted_once");
}

static void _perm_allow_foreground_cb(lv_event_t *e)
{
    _perm_button_cb(e, COS_PERM_STATE_ALLOW_FOREGROUND, "granted_foreground");
}

static void _perm_deny_cb(lv_event_t *e)
{
    _perm_button_cb(e, COS_PERM_STATE_DENIED, "denied");
}
