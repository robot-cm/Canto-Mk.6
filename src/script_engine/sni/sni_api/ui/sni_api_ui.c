 ...
 * ArcList/LayoutManager Home) to JavaScript as `eos.ui.*`. A UI written in JS
                                               ...
                                               const jerry_length_t args_count)
{
    ...
    eos_profile_id_t id = (eos_profile_id_t)(int32_t)jerry_value_as_number(args_p[0]);
    eos_display_profile_t *p = eos_malloc(sizeof(*p));
    if (!p)
        return sni_api_throw_error("Out of memory");
    *p = eos_display_profiles_get(id);
    return sni_tb_c2js(&p, SNI_H_EOS_UI_PROFILE);
}
                                             ...
                                             const jerry_length_t args_count)
{
    ...
    eos_display_shape_t shape = (eos_display_shape_t)(int32_t)jerry_value_as_number(args_p[0]);
    ...
    if (!p)
        return sni_api_throw_error("Out of memory");
    *p = eos_display_profile_create(shape, w, h);
    return sni_tb_c2js(&p, SNI_H_EOS_UI_PROFILE);
}
                                          ...
                                          const jerry_length_t args_count)
{
    ...
    eos_display_profile_t *p = NULL;
    if (!sni_tb_js2c(args_p[0], SNI_H_EOS_UI_PROFILE, &p))
                                                  ...
                                                  const jerry_length_t args_count)
{
    ...
    return jerry_boolean(eos_display_profile_is_inside(p, x, y, r));
}
                                     ...
                                     const jerry_length_t args_count)
{
    ...
    eos_display_profile_t *p = NULL;
    if (!sni_tb_js2c(args_p[1], SNI_H_EOS_UI_PROFILE, &p))
        return sni_api_throw_error("Invalid profile handle");
    ...
    const char **names = eos_malloc((n ? n : 1) * sizeof(char *));
    if (!names)
        return sni_api_throw_error("Out of memory");
    ...
    eos_framework_home_t *native =
        eos_framework_home_create(view, p, (const char **)names, (int)n, NULL);

    /* Names are copied into LVGL labels by the view; free the temp array. */
    for (uint32_t i = 0; i < n; i++)
        if (names[i]) eos_free((void *)names[i]);
    eos_free(names);
    ...
    sni_ui_home_t *wrap = eos_malloc(sizeof(*wrap));
    if (!wrap)
    {
        eos_framework_home_destroy(native);
        jerry_value_free(on_select);
        return sni_api_throw_error("Out of memory");
    }
    ...
    eos_framework_home_set_on_select(native, sni_api_ui_home_on_select, wrap);
    return sni_tb_c2js(&wrap, SNI_H_EOS_UI_HOME);
}
                                             ...
                                             const jerry_length_t args_count)
{
    ...
    if (!sni_tb_js2c(args_p[0], SNI_H_EOS_UI_HOME, &wrap))
    ...
    if (wrap->home)
    {
        ...
        lv_anim_del_all();
        eos_framework_home_destroy(wrap->home);
        wrap->home = NULL;
    }
                                                    ...
                                                    const jerry_length_t args_count)
{
    ...
    float x, y, w, h;
    eos_framework_home_statusbar_rect(wrap->home, &x, &y, &w, &h);
    jerry_value_t obj = jerry_object();
                                           ...
                                           const jerry_length_t args_count)
{
    ...
    if (!sni_tb_js2c(args_p[0], SNI_H_EOS_UI_HOME, &wrap))
        return sni_api_throw_error("Invalid home handle");
    return jerry_number((double)eos_framework_home_count(wrap->home));
}
                                                 ...
                                                 const jerry_length_t args_count)
{
    ...
    int cnt = eos_framework_home_count(wrap->home);
    if (i < 0 || i >= cnt)
        return jerry_undefined();

    float x, y;
    eos_framework_home_item_center(wrap->home, i, &x, &y);
    jerry_value_t obj = jerry_object();
    script_engine_set_prop_number(obj, "x", (double)x);
    script_engine_set_prop_number(obj, "y", (double)y);
    return obj;
                                          ...
                                          const jerry_length_t args_count)
{
    ...
    if (!sni_tb_js2c(args_p[0], SNI_H_EOS_UI_HOME, &wrap))
        return sni_api_throw_error("Invalid home handle");
    eos_framework_home_step(wrap->home, (float)jerry_value_as_number(args_p[1]));
    return jerry_undefined();
}
                                          ...
                                          const jerry_length_t args_count)
{
    ...
    if (!sni_tb_js2c(args_p[0], SNI_H_EOS_UI_HOME, &wrap))
        return sni_api_throw_error("Invalid home handle");
    eos_framework_home_drag(wrap->home, (float)jerry_value_as_number(args_p[1]));
    return jerry_undefined();
}
                                             ...
                                             const jerry_length_t args_count)
{
    ...
    if (!sni_tb_js2c(args_p[0], SNI_H_EOS_UI_HOME, &wrap))
        return sni_api_throw_error("Invalid home handle");
    eos_framework_home_release(wrap->home);
    return jerry_undefined();
}
...
static const sni_constant_desc_t ui_constants[] = {
    {.name = "PROFILE_COUNT",   .type = SNI_CONST_INT, .value.i = EOS_PROFILE_COUNT},
    {.name = "SHAPE_CIRCLE",    .type = SNI_CONST_INT, .value.i = EOS_DISPLAY_SHAPE_CIRCLE},
    {.name = "SHAPE_SQUARE",    .type = SNI_CONST_INT, .value.i = EOS_DISPLAY_SHAPE_SQUARE},
    {.name = "SHAPE_RECTANGLE", .type = SNI_CONST_INT, .value.i = EOS_DISPLAY_SHAPE_RECTANGLE},
...
void sni_api_ui_init(void)
{
    ...
    {
        EOS_LOG_E("[%s] failed to create ui API object", EOS_UI_LOG_TAG);
        return;
    }
    ui_register_methods(ui_static_methods, ui_api_obj);
    if (!sni_api_register_constants(ui_constants, ui_api_obj))
    {
        EOS_LOG_E("[%s] failed to register ui constants", EOS_UI_LOG_TAG);
    }
}

void sni_api_ui_mount(jerry_value_t realm)
{
    jerry_value_t eos_obj = jerry_object_get_sz(realm, "eos");
    if (!jerry_value_is_object(eos_obj))
    {
        EOS_LOG_E("[%s] eos namespace not found in realm", EOS_UI_LOG_TAG);
        jerry_value_free(eos_obj);
        return;
    }
    jerry_value_t set = jerry_object_set_sz(eos_obj, "ui", ui_api_obj);
    jerry_value_free(set);
    jerry_value_free(eos_obj);
}
