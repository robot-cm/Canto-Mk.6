 ...
 * native-app registry (cos_app_list.{h,c}) exactly like Album/Texthub.
 *
 * Removability: comment out COS_APP_EQSOLVER_ENABLED below (and the matching
 * #ifdef guards in cos_app_list.c) to compile the whole app out.
 */

#ifndef COS_EQSOLVER_H
#define COS_EQSOLVER_H
...
#define COS_APP_EQSOLVER_ENABLED

/* App icon (must be deployed to this sdcard path, same scheme as other apps). */
#define COS_IMG_EQSOLVER "/sdcard/theme/icons/eqsolver.bin"
