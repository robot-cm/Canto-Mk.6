...
static char (*s_paths)[COS_FS_PATH_MAX]; /* viewer list of full paths (PSRAM) */
static int   s_count;
static int   s_index;
...
static bool _png_dim(const char *path, uint32_t *w, uint32_t *h)
{
    ...
    if (fp == COS_FILE_INVALID) return false;

    uint8_t buf[24];
    if (cos_storage_file_read(fp, buf, sizeof(buf)) != (ssize_t)sizeof(buf)) {
...
static char s_hist_img[COS_FS_PATH_MAX];
static char s_fm_anchor[COS_FS_PATH_MAX];

/* FM 展开记录:上次 FM 打开时展开过的目录(最多 FM_EXP_MAX 个,行尾最新) */
static char s_fm_exp[FM_EXP_MAX][COS_FS_PATH_MAX];
static int  s_fm_exp_count;

static void _history_flush(void)
{
    char buf[COS_FS_PATH_MAX * 6 + 16];
    ...
    cos_storage_mkdir_recursive(ALBUM_HIST_DIR);
    cos_storage_write_file(ALBUM_HIST_FILE, buf, strlen(buf));
}
...
static bool _read_history(char *out, size_t out_size)
{
    ...
    if (!cos_storage_is_file(ALBUM_HIST_FILE)) return false;

    char *content = cos_storage_read_file(ALBUM_HIST_FILE);
...
static size_t _jd_in(JDEC *jd, uint8_t *buf, size_t nd)
{
    ...
    if (!buf) { /* skip nd bytes */
        uint32_t pos = 0;
        if (cos_storage_file_tell(c->fp, &pos) != COS_OK) {
            COS_LOG_E("Album: skip tell FAIL");
            return 0;
        }
        if (cos_storage_file_seek(c->fp, pos + nd) != COS_OK) {
            COS_LOG_E("Album: skip seek FAIL pos=%u nd=%u", pos, (unsigned)nd);
            return 0;
        }
        return nd;
    }
    ssize_t rd = cos_storage_file_read(c->fp, buf, nd);
...
static bool _decode_jpeg(const char *path, lv_image_dsc_t *dsc)
{
    ...
    if (fp == COS_FILE_INVALID) {
        COS_LOG_E("Album: open FAIL %s", path);
        return false;
    }
    COS_LOG_I("Album: decode %s", path);

    /* TJpgDec working pool (huffman tables + MCU buffer), PSRAM to save IRAM */
    uint8_t *work = (uint8_t *)cos_malloc(ALBUM_WORK_SIZE);
    if (!work) {
        COS_LOG_E("Album: work malloc FAIL size=%u", (unsigned)ALBUM_WORK_SIZE);
        cos_storage_file_close(fp);
        return false;
    }
    ...
    if (rc != JDR_OK) {
        COS_LOG_E("Album: jd_prepare FAIL rc=%d", (int)rc);
        cos_free(work);
        cos_storage_file_close(fp);
        return false;
    }
    ...
    if (ow == 0 || oh == 0 || ow > ALBUM_DECODE_MAX || oh > ALBUM_DECODE_MAX) {
        COS_LOG_E("Album: jpeg %ux%u too large even at 1/8", w, h);
        cos_free(work);
        cos_storage_file_close(fp);
    ...
    uint8_t *pix = (uint8_t *)cos_malloc(npix);
    if (!pix) {
        COS_LOG_E("Album: pix malloc FAIL size=%u", (unsigned)npix);
        cos_free(work);
        cos_storage_file_close(fp);
        return false;
    }
    ...
    COS_LOG_I("Album: jpeg %ux%u scale=%u -> %ux%u", w, h, scale, ow, oh);

    JRESULT drc = jd_decomp(&jd, _jd_out, scale);
    if (drc != JDR_OK) {
        COS_LOG_E("Album: jd_decomp FAIL rc=%d w=%u h=%u scale=%u", (int)drc, w, h, scale);
        cos_free(pix);
        cos_free(work);
        cos_storage_file_close(fp);
        return false;
...
static int _load_dir_pics(const char *dir)
{
    ...
    if (!cos_storage_is_dir(dir)) return 0;

    s_paths = (char (*)[COS_FS_PATH_MAX])cos_malloc(ALBUM_MAX * COS_FS_PATH_MAX);
    if (!s_paths) return 0;
    ...
    while (s_count < ALBUM_MAX &&
        ...
        if (cos_storage_is_file(full) && _is_image_ext(name)) {
    ...
    if (s_count > 1) {
        qsort(s_paths, s_count, COS_FS_PATH_MAX, _cmp_path);
    }
    return s_count;
}
...
{
    char dir[COS_FS_PATH_MAX], base[COS_FS_PATH_MAX];
...
/* safe path scratch buffer: longest album path (COS_FS_PATH_MAX) + mirror
...
#define ALBUM_PATH_BUF (COS_FS_PATH_MAX + 64)
 ...
 * buffer base in *alloc — free it with cos_free(). */
                      ...
                      uint32_t *npix_out)
{
    ...
    uint8_t *buf = (uint8_t *)cos_malloc(12 + npix);
    if (!buf) {
        cos_storage_file_close(fp);
        return false;
    }
    memcpy(buf, hdr, sizeof(hdr));
    if (cos_storage_file_read(fp, buf + sizeof(hdr), npix) != (ssize_t)npix) {
        cos_free(buf);
        cos_storage_file_close(fp);
        return false;
    }
...
static void _tile_unload(_tile_t *t)
{
    ...
    if (t->pixels) {
        cos_free(t->pixels);           /* give PSRAM back immediately */
        t->pixels = NULL;
    }
    t->used = false;
}
...
#pragma GCC diagnostic pop
    ...
    if (!_load_bin(path, &dsc, &buf, NULL)) {
        COS_LOG_W("Album: tile %d,%d load FAIL", c, r);
        return;
    }
    _tile_t *t = NULL;
    for (int i = 0; i < ALBUM_TILE_MAX; i++) {
    ...
    if (!t) {                          /* no slot: drop, memory stays bounded */
        cos_free(buf);
        COS_LOG_W("Album: tile slot full (%d,%d)", c, r);
        return;
    }
    t->c = c;
    t->r = r;
...
#pragma GCC diagnostic pop
    char *meta = cos_storage_read_file(mpath);
    if (!meta) {
...
#pragma GCC diagnostic pop
    ...
    _tile_refresh();
    COS_LOG_I("Album: %d.%dx enc %dx%d grid %dx%d ts=%d view=%d,%d",
...
static bool _fm_node_load(_fm_node_t *n)
{
    ...
    if (!d) {
        n->loaded = true;
        COS_LOG_W("Album: opendir FAIL %s", n->path);
        return false;
    }
    int cnt = 0;
    char name[COS_FS_NAME_MAX];
    ...
    cos_storage_dir_close(d);
    COS_LOG_I("Album: dir %s raw=%d", n->path, cnt);

    if (cnt == 0) { n->loaded = true; return false; }

    _fm_node_t *arr = (_fm_node_t *)cos_malloc_zeroed((size_t)cnt * sizeof(_fm_node_t));
    ...
    COS_LOG_I("Album: dir %s children=%d", n->path, i);
    return i > 0;
}
...
static void _fm_rebuild(void)
{
    if (!s_fm_open || !s_fm_list) {
        COS_LOG_W("Album: rebuild skip open=%d list=%p", s_fm_open, (void *)s_fm_list);
        return;
    }
    ...
    if (!s_fm_root) {
        COS_LOG_E("Album: rebuild root NULL");
        return;
    }
    if (!s_fm_root->loaded) _fm_node_load(s_fm_root);

    if (!s_fm_items) {
        s_fm_items = (_fm_item_t *)cos_malloc(FM_MAX_VISIBLE * sizeof(_fm_item_t));
        if (!s_fm_items) {
            COS_LOG_E("Album: items alloc FAIL size=%u",
...
static void _fm_open(void)
{
    ...
    cos_label_set_font_size(s_fm_title, COS_FONT_SIZE_MICRO);
    ...
    /* root node */
    s_fm_root = (_fm_node_t *)cos_malloc_zeroed(sizeof(_fm_node_t));
    if (s_fm_root) {
        ...
        COS_LOG_E("Album: root alloc FAIL size=%u", (unsigned)sizeof(_fm_node_t));
    }
...
static void _delete_assets(const char *img)
{
    ...
    if (cos_storage_is_file(p)) cos_storage_file_remove(p);

    _tiles_dir(img, p, sizeof(p));        /* <dir>/.tiles/<base>/ */
    cos_dir_t d = cos_storage_dir_open(p);
    if (!d) return;
    char name[COS_FS_NAME_MAX];
    while (cos_storage_dir_read(d, name, sizeof(name)) == COS_OK) {
        if (name[0] == '\0' || name[0] == '.') continue;
        ...
        if (cos_storage_is_file(full)) cos_storage_file_remove(full);
    }
    cos_storage_dir_close(d);
}
...
static void _trash_cb(lv_event_t *e)
{
    ...
    const char *path = s_paths[s_index];
    COS_LOG_I("Album: delete %s", path);
    cos_storage_file_remove(path);
 ...
 * 缩略图态右滑仍按系统默认退出。 */
static bool _swipe_back_cb(cos_activity_t *self, lv_dir_t dir)
{
    (void)self;
    (void)dir;
    return s_tile_mode;
...
{
    s_act = act;
    cos_activity_set_swipe_back_handler(act, _swipe_back_cb);
    _build_ui(act);

    /* resume last viewed image; otherwise open the file manager */
    char hist[COS_FS_PATH_MAX];
