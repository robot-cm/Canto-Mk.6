/**
 * @file cos_test_event.c
 * @brief Comprehensive event system test module
 */

#include "cos_test_event.h"
...
static cos_event_code_t _test_event_id = COS_EVENT_UNKNOWN;
...
static void _test_event_cb(cos_event_t *e)
{
    _cb_call_count++;
    _cb_last_user_data = cos_event_get_user_data(e);
    _cb_last_param = cos_event_get_param(e);
    _cb_last_obj = cos_event_get_obj(e);
}

static void _test_event_cb2(cos_event_t *e)
{
    _cb_call_count++;
    (void)e;
}
...
{
    _test_event_id = cos_event_register_id();

    bool passed = (_test_event_id >= COS_EVENT_LAST);
...
static bool _test_event_subscribe_basic(void)
{
    ...
    cos_event_subscribe(_test_event_id, _test_event_cb, test_data);
    cos_event_post(_test_event_id, NULL, NULL);

    bool passed = (_cb_call_count == 1);

    cos_event_unsubscribe(_test_event_id, _test_event_cb);
...
static bool _test_event_subscribe_ex(void)
{
    ...
    cos_event_subscribe_ex(_test_event_id, _test_event_cb, test_data, test_obj);
    cos_event_post(_test_event_id, NULL, test_obj);

    bool passed = (_cb_call_count == 1 && _cb_last_obj == test_obj);

    cos_event_unsubscribe_with_obj(_test_event_id, _test_event_cb, test_obj);
...
static bool _test_event_callback_data(void)
{
    ...
    cos_event_subscribe(_test_event_id, _test_event_cb, user_data);
    cos_event_post(_test_event_id, param, NULL);
...
static bool _test_event_unsubscribe_basic(void)
{
    ...
    cos_event_subscribe(_test_event_id, _test_event_cb, NULL);
    cos_event_post(_test_event_id, NULL, NULL);
    cos_event_unsubscribe(_test_event_id, _test_event_cb);
    cos_event_post(_test_event_id, NULL, NULL);
...
static bool _test_event_unsubscribe_with_user_data(void)
{
    ...
    cos_event_subscribe(_test_event_id, _test_event_cb, data1);
    cos_event_subscribe(_test_event_id, _test_event_cb, data2);

    cos_event_post(_test_event_id, NULL, NULL);
    int count_after_first_post = _cb_call_count;

    cos_event_unsubscribe_with_user_data(_test_event_id, _test_event_cb, data1);
    cos_event_post(_test_event_id, NULL, NULL);

    bool passed = (count_after_first_post == 2 && _cb_call_count == 3);

    cos_event_unsubscribe_with_user_data(_test_event_id, _test_event_cb, data2);
...
static bool _test_event_unsubscribe_all(void)
{
    ...
    cos_event_subscribe(_test_event_id, _test_event_cb, NULL);
    cos_event_subscribe(_test_event_id, _test_event_cb2, NULL);
    cos_event_subscribe(COS_EVENT_LANGUAGE_CHANGED, _test_event_cb, NULL);

    cos_event_post(_test_event_id, NULL, NULL);
    int count_before_unsubscribe = _cb_call_count;

    cos_event_unsubscribe_all(_test_event_cb);
    cos_event_post(_test_event_id, NULL, NULL);

    bool passed = (count_before_unsubscribe == 2 && _cb_call_count == 3);

    cos_event_unsubscribe_all(_test_event_cb2);
...
static bool _test_event_unsubscribe_with_obj(void)
{
    ...
    cos_event_subscribe_ex(_test_event_id, _test_event_cb, NULL, obj1);
    cos_event_subscribe_ex(_test_event_id, _test_event_cb, NULL, obj2);

    cos_event_post(_test_event_id, NULL, NULL);
    int count_after_first_post = _cb_call_count;

    cos_event_unsubscribe_with_obj(_test_event_id, _test_event_cb, obj1);
    cos_event_post(_test_event_id, NULL, NULL);

    bool passed = (count_after_first_post == 2 && _cb_call_count == 3);

    cos_event_unsubscribe_with_obj(_test_event_id, _test_event_cb, obj2);
    lv_obj_delete(obj1);
    lv_obj_delete(obj2);
...
static bool _test_event_internal_events(void)
{
    ...
    cos_event_unsubscribe(COS_EVENT_LANGUAGE_CHANGED, _test_event_cb);
...
static bool _test_event_multiple_subscribers(void)
{
    ...
    cos_event_unsubscribe(_test_event_id, _test_event_cb);
    cos_event_unsubscribe(_test_event_id, _test_event_cb2);
...
static bool _test_event_null_safety(void)
{
    ...
    cos_event_subscribe(COS_EVENT_UNKNOWN, NULL, NULL);
    cos_event_unsubscribe(COS_EVENT_UNKNOWN, NULL);
    cos_event_post(COS_EVENT_UNKNOWN, NULL, NULL);
    cos_event_unsubscribe_all(NULL);
...
static bool _test_event_obj_payload(void)
{
    ...
    cos_event_subscribe_ex(_test_event_id, _test_event_cb, NULL, test_obj);
    cos_event_post(_test_event_id, NULL, test_obj);
...
static cos_activity_lifecycle_t s_event_test_activity_lifecycle;

void cos_test_event_start(void)
{
    cos_activity_t *activity = cos_activity_create(&s_event_test_activity_lifecycle);
    if (!activity)
    {
        return;
    }
    ...
    cos_activity_set_title(activity, "Event Tests");
...
{
    cos_test_register("Event: register dynamic ID", _test_event_register_id);
    cos_test_register("Event: basic subscribe/post", _test_event_subscribe_basic);
    cos_test_register("Event: subscribe with obj", _test_event_subscribe_ex);
    cos_test_register("Event: callback data passing", _test_event_callback_data);
    cos_test_register("Event: basic unsubscribe", _test_event_unsubscribe_basic);
    cos_test_register("Event: unsubscribe with user data", _test_event_unsubscribe_with_user_data);
    cos_test_register("Event: unsubscribe all", _test_event_unsubscribe_all);
    cos_test_register("Event: unsubscribe with obj", _test_event_unsubscribe_with_obj);
    cos_test_register("Event: internal events", _test_event_internal_events);
    cos_test_register("Event: multiple subscribers", _test_event_multiple_subscribers);
    cos_test_register("Event: cleanup no crash", _test_event_cleanup);
    cos_test_register("Event: null safety", _test_event_null_safety);
    cos_test_register("Event: obj payload", _test_event_obj_payload);
}
