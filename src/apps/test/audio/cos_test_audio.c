...
{
    cos_result_t ret = cos_dev_speaker_register(NULL);
    bool passed = (ret != COS_OK);
...
{
    cos_dev_speaker_ops_t ops = {0};
    /* All required fields are NULL */
    cos_result_t ret = cos_dev_speaker_register(&ops);
    bool passed = (ret == COS_ERR_INVALID_ARG || ret == COS_ERR_ALREADY_EXISTS);
...
static bool _test_spk_register_duplicate(void)
{
    ...
    cos_result_t ret = cos_dev_speaker_register(spk->ops);
    bool passed = (ret == COS_ERR_ALREADY_EXISTS);
    _record_test("Speaker: reject duplicate register",
                 passed,
                 passed ? "Duplicate rejected with COS_ERR_ALREADY_EXISTS" : "Duplicate should have been rejected");
    return passed;
}
...
static bool _test_spk_state_transitions(void)
{
    ...
    cos_dev_state_t initial = cos_dev_speaker_get_state();
    ...
    cos_dev_speaker_report_state(DEV_STATE_BUSY);
    cos_dev_state_t busy = cos_dev_speaker_get_state();

    cos_dev_speaker_report_state(DEV_STATE_READY);
    cos_dev_state_t back = cos_dev_speaker_get_state();
...
{
    cos_dev_state_t state = cos_dev_microphone_get_state();
    bool passed = (state == DEV_STATE_READY);
...
{
    cos_result_t ret = cos_dev_microphone_register(NULL);
    bool passed = (ret != COS_OK);
...
{
    cos_dev_microphone_ops_t ops = {0};
    cos_result_t ret = cos_dev_microphone_register(&ops);
    bool passed = (ret == COS_ERR_INVALID_ARG || ret == COS_ERR_ALREADY_EXISTS);
...
static bool _test_mic_register_duplicate(void)
{
    ...
    cos_result_t ret = cos_dev_microphone_register(mic->ops);
    bool passed = (ret == COS_ERR_ALREADY_EXISTS);
...
{
    cos_dev_state_t initial = cos_dev_microphone_get_state();
    if (initial != DEV_STATE_READY)
    {
    ...
    cos_dev_microphone_report_state(DEV_STATE_BUSY);
    cos_dev_state_t busy = cos_dev_microphone_get_state();

    cos_dev_microphone_report_state(DEV_STATE_READY);
    cos_dev_state_t back = cos_dev_microphone_get_state();
...
{
    cos_result_t ret = cos_service_audio_set_volume(50);
    bool passed = (ret == COS_OK);
...
{
    cos_result_t ret = cos_service_audio_set_volume(0);
    bool passed = (ret == COS_OK);
...
{
    cos_result_t ret = cos_service_audio_set_volume(100);
    bool passed = (ret == COS_OK);
...
{
    cos_result_t ret = cos_service_audio_set_volume(200);
...
{
    cos_service_audio_set_volume(75);
    uint8_t vol = cos_service_audio_get_volume();
    bool passed = (vol == 75);
...
{
    cos_service_audio_set_volume(60);
    cos_service_audio_set_mute(true);
    bool is_muted = cos_service_audio_is_muted();
...
{
    cos_result_t ret = cos_service_audio_play(NULL);
    bool passed = (ret == COS_ERR_INVALID_ARG);
    _record_test("Service: play NULL path rejected",
                 passed,
                 passed ? "NULL path returns COS_ERR_INVALID_ARG" : "Should reject NULL");
    return passed;
}
...
{
    cos_result_t ret = cos_service_audio_play("/fs/music.mp3");
    /* Accept either COS_OK or COS_ERR_DEV_ERROR (file may not exist
     * on host, or audio format may be unsupported). The important
     * thing is the call doesn't crash and returns a valid error code. */
    bool passed = (ret == COS_OK || ret == COS_ERR_DEV_ERROR || ret == COS_ERR_NOT_FOUND);
...
{
    cos_result_t ret = cos_service_audio_stop();
    bool passed = (ret == COS_OK);
...
/**
 * @brief Service: play_tone when ops is NULL returns COS_ERR_DEV_OPS_NOT_SUPPORTED
 */
static bool _test_service_play_tone_unsupported(void)
{
    cos_result_t ret = cos_service_audio_play_tone(440, 500);
    bool passed = (ret == COS_ERR_DEV_OPS_NOT_SUPPORTED);
    _record_test("Service: play_tone unsupported",
                 passed,
                 passed ? "Returns COS_ERR_DEV_OPS_NOT_SUPPORTED" : "Should report unsupported");
    return passed;
}
...
{
    cos_result_t ret = cos_service_audio_start_recording(NULL);
    bool passed = (ret == COS_ERR_INVALID_ARG);
...
{
    cos_result_t ret = cos_service_audio_start_recording(RECORD_TEST_PATH);
    /* Accept COS_OK, or COS_ERR_DEV_ERROR (permission denied / no input device) */
    bool passed = (ret == COS_OK || ret == COS_ERR_DEV_ERROR);
    _record_test("Service: start recording",
                 passed,
                 passed
                     ? (ret == COS_OK ? "Recording started" : "Permission/mic not available (expected on some systems)")
...
{
    cos_result_t ret = cos_service_audio_stop_recording();
...
{
    cos_result_t ret = cos_service_audio_play(RECORD_TEST_PATH);
    /* Accept COS_OK or COS_ERR_DEV_ERROR (file may not exist if recording failed) */
    bool passed = (ret == COS_OK || ret == COS_ERR_DEV_ERROR);
...
{
    bool avail = cos_service_audio_speaker_available();
...
{
    bool avail = cos_service_audio_microphone_available();
    bool passed = (avail == true);
...
{
    cos_service_audio_set_volume(30);
    cos_result_t ret1 = cos_service_audio_set_volume(30);
    bool passed = (ret1 == COS_OK);
...
{
    cos_dev_speaker_t *spk1 = cos_dev_speaker_get_instance();
    cos_dev_speaker_t *spk2 = cos_dev_speaker_get_instance();
...
{
    cos_dev_microphone_t *mic1 = cos_dev_microphone_get_instance();
    cos_dev_microphone_t *mic2 = cos_dev_microphone_get_instance();
...
{
    cos_dev_speaker_report_state(DEV_STATE_ERROR);
    cos_dev_state_t state = cos_dev_speaker_get_state();
...
{
    cos_dev_microphone_report_state(DEV_STATE_ERROR);
    cos_dev_state_t state = cos_dev_microphone_get_state();
    bool passed = (state == DEV_STATE_ERROR);
    cos_dev_microphone_report_state(DEV_STATE_READY);
...
{
    cos_test_register("Speaker: get_instance non-null", _test_spk_get_instance);
    cos_test_register("Speaker: get_instance consistent", _test_spk_get_instance_consistent);
    cos_test_register("Speaker: state is READY", _test_spk_initial_state);
    cos_test_register("Speaker: reject NULL ops", _test_spk_register_null_ops);
    cos_test_register("Speaker: reject incomplete ops", _test_spk_register_incomplete_ops);
    cos_test_register("Speaker: reject missing open", _test_spk_register_missing_open);
    cos_test_register("Speaker: reject missing is_available", _test_spk_register_missing_is_available);
    cos_test_register("Speaker: register with optional NULL ops", _test_spk_register_required_only);
    cos_test_register("Speaker: reject duplicate register", _test_spk_register_duplicate);
    cos_test_register("Speaker: state transitions", _test_spk_state_transitions);
    cos_test_register("Speaker: report_state same no-op", _test_spk_report_state_same);
    cos_test_register("Speaker: report_state ERROR", _test_spk_report_state_error);
    cos_test_register("Mic: get_instance non-null", _test_mic_get_instance);
    cos_test_register("Mic: get_instance consistent", _test_mic_get_instance_consistent);
    cos_test_register("Mic: state is READY", _test_mic_initial_state);
    cos_test_register("Mic: reject NULL ops", _test_mic_register_null_ops);
    cos_test_register("Mic: reject incomplete ops", _test_mic_register_incomplete_ops);
    cos_test_register("Mic: reject missing set_buffer", _test_mic_register_missing_set_buffer);
    cos_test_register("Mic: reject missing is_available", _test_mic_register_missing_is_available);
    cos_test_register("Mic: register with optional NULL ops", _test_mic_register_required_only);
    cos_test_register("Mic: reject duplicate register", _test_mic_register_duplicate);
    cos_test_register("Mic: state transitions", _test_mic_state_transitions);
    cos_test_register("Mic: report_state ERROR", _test_mic_report_state_error);
    cos_test_register("Service: set_volume 50", _test_service_set_volume_valid);
    cos_test_register("Service: set_volume 0 (min)", _test_service_set_volume_min);
    cos_test_register("Service: set_volume 100 (max)", _test_service_set_volume_max);
    cos_test_register("Service: set_volume 200 clamped to 100", _test_service_set_volume_clamped);
    cos_test_register("Service: set_volume idempotent", _test_service_set_volume_idempotent);
    cos_test_register("Service: get_volume after set", _test_service_get_volume);
    cos_test_register("Service: default mute = false", _test_service_mute_default);
    cos_test_register("Service: mute on", _test_service_mute_on);
    cos_test_register("Service: mute off", _test_service_mute_off);
    cos_test_register("Service: play NULL path rejected", _test_service_play_null);
    cos_test_register("Service: play file API works", _test_service_play_valid);
    cos_test_register("Service: play_tone unsupported", _test_service_play_tone_unsupported);
    cos_test_register("Service: stop", _test_service_stop);
    cos_test_register("Service: stop when idle", _test_service_stop_idle);
    cos_test_register("Service: record NULL path rejected", _test_service_record_null);
    cos_test_register("Service: start recording", _test_service_record_start);
    cos_test_register("Service: stop recording", _test_service_record_stop);
    cos_test_register("Service: play back recording", _test_service_record_playback);
    cos_test_register("Service: speaker available", _test_service_speaker_available);
    cos_test_register("Service: microphone available", _test_service_microphone_available);
}

void cos_test_audio_start(void)
{
    cos_test_audio_register_tests();
    cos_test_fw_page_start("Audio Tests");
}
