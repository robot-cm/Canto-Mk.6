...
static void _password_close_cb(const char *text, eos_input_result_t result, void *user_data);
...
static void _password_close_cb(const char *text, eos_input_result_t result, void *user_data)
{
    ...
    if (result == EOS_INPUT_RESULT_CANCEL)
    {
    ...
    case _PASSWORD_STAGE_CONFIRM:
        if (digits[0] == '\0')
        {
            eos_toast_show_char_icon(RI_ERROR_WARNING_FILL, EOS_COLOR_RED, "Password cannot be empty");
            state->stage = _PASSWORD_STAGE_NEW;
            lv_async_call(_password_open_next_async, state);
            break;
        }
...
static void _password_open_next_async(void *user_data)
{
    ...
    if (!state)
        return;
    eos_input_page_open_with_callback(NULL, _password_close_cb, state);
}
