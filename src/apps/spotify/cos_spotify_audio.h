 ...
 *     每次最多解码 COS_SPOTIFY_DECODE_SLICE_MS 毫秒的音频,保证 ui_task
 ...
 *   · 所有 SD 访问经 cos_storage_* 封装,禁止裸 fopen/sdmmc_*(任务书 §3.3)。
 */
#ifndef COS_SPOTIFY_AUDIO_H
#define COS_SPOTIFY_AUDIO_H
...
#define COS_SPOTIFY_PCM_RING_BYTES     (32 * 1024)

/** 单次解码分片的时间预算(ms):控制单次 lv_timer 回调的占用时长。 */
#define COS_SPOTIFY_DECODE_SLICE_MS    8

/** 支持的最大单文件尺寸,超出直接拒绝(防 SD 上的异常文件)。 */
#define COS_SPOTIFY_MAX_FILE_BYTES     (64u * 1024u * 1024u)

typedef enum
{
    COS_SPOTIFY_FMT_UNKNOWN = 0,
    COS_SPOTIFY_FMT_MP3,
    COS_SPOTIFY_FMT_WAV,
} cos_spotify_fmt_t;
...
typedef enum
{
    COS_SPOTIFY_ADE_OK = 0,
    COS_SPOTIFY_ADE_ERR_OPEN,      /* 文件打不开                       */
    COS_SPOTIFY_ADE_ERR_FORMAT,    /* 非 MP3/WAV 或格式不支持          */
    COS_SPOTIFY_ADE_ERR_MEM,       /* 内存不足                         */
    COS_SPOTIFY_ADE_ERR_DECODE,    /* 解码器报错                       */
} cos_spotify_ade_err_t;
 ...
 * COS_CONFIG_KEY_SPEAKER_VOLUME_NUMBER 做持久化,保证与系统静音策略统一。 */
