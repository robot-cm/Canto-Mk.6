...
extern const lv_image_dsc_t eos_icon_star;
...
static void _stars_free_data(void)
{
    for (int i = 0; i < s_stars_count; i++) {
        if (s_stars_paths) eos_free(s_stars_paths[i]);
    }
    eos_free(s_stars_paths);    s_stars_paths = NULL;
    eos_free(s_stars_active);   s_stars_active = NULL;
    eos_free(s_stars_star_img); s_stars_star_img = NULL;
    s_stars_count = 0;
}
...
static void _stars_write_file(void)
{
    ...
    eos_storage_write_file(TH_STARS_FILE, buf, (size_t)n);
    eos_free(buf);
}
...
static void _th_stars_append(const char *path)
{
    ...
    char *buf = NULL;
    size_t len = 0;
    if (eos_storage_is_file(TH_STARS_FILE)) {
        buf = eos_storage_read_file(TH_STARS_FILE);
        if (buf) {
            if (_th_stars_contains(buf, path)) {
                EOS_LOG_I("Texthub: already starred %s", path);
                eos_free(buf);
                return;
            }
            len = strlen(buf);
    ...
    char *out = (char *)eos_malloc(len + add + 1);
    if (!out) { eos_free(buf); return; }
    ...
    eos_storage_write_file(TH_STARS_FILE, out, n);
    eos_free(out);
    eos_free(buf);
}
...
{
    _stars_free_data();
    if (!eos_storage_is_file(TH_STARS_FILE)) return;
    char *buf = eos_storage_read_file(TH_STARS_FILE);
    if (!buf) return;
    ...
    if (cnt == 0) { eos_free(buf); return; }

    char **paths = (char **)eos_malloc((size_t)cnt * sizeof(char *));
    bool  *act   = (bool  *)eos_malloc((size_t)cnt * sizeof(bool));
    if (!paths || !act) {
        eos_free(paths); eos_free(act);
        eos_free(buf);
        return;
    }
    ...
    for (char *p = buf; *p && i < cnt; ) {
        ...
        while (len > 0 && (p[len - 1] == '\r' || p[len - 1] == ' '
                        || p[len - 1] == '\t')) len--;
        if (len > 0) {
            paths[i] = (char *)eos_malloc((size_t)len + 1);
            if (paths[i]) {
...
static void _stars_arrow_cb(lv_event_t *e)
{
    ...
    if (_open_path(path)) {
        ...
        EOS_LOG_W("Texthub: stars open fail %s", path);
    }
}
...
static void _stars_rebuild(void)
{
    ...
    if (s_stars_count == 0) {
        ...
        eos_label_set_font_size(msg, EOS_FONT_SIZE_MICRO);
    ...
    eos_free(s_stars_star_img);
    s_stars_star_img = (lv_obj_t **)eos_malloc((size_t)s_stars_count * sizeof(lv_obj_t *));
...
static void _stars_fab_cb(lv_event_t *e)
{
    ...
    EOS_LOG_I("Texthub: starred %s", s_cur_path);
}
...
static void _stars_open(void)
{
    ...
    eos_label_set_font_size(hdr, EOS_FONT_SIZE_MICRO);
