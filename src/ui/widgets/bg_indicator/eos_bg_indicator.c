...
#define EOS_LOG_TAG "BgIndicator"

/* Entry for one background-running app -----------------------*/
typedef struct eos_bg_indicator_entry
{
    char app_id[64];
    char icon_src[EOS_FS_PATH_MAX];
    eos_activity_t *activity;
    bool running;
    struct eos_bg_indicator_entry *next;
} eos_bg_indicator_entry_t;

/* Static variables -------------------------------------------*/
static eos_bg_indicator_entry_t *_s_list = NULL;
...
static void _icon_clicked_cb(lv_event_t *e)
{
    ...
    eos_bg_indicator_entry_t *entry = (eos_bg_indicator_entry_t *)lv_obj_get_user_data(icon);
    if (!entry)
        return;

    EOS_LOG_I("BgIndicator icon clicked: app_id=%s", entry->app_id);

    eos_activity_t *activity = entry->activity;
    ...
    eos_bg_indicator_unregister(entry->app_id);

    /* Bring the app back to the foreground. */
    if (activity)
        eos_activity_restore_background(activity);
}
...
static void _rebuild_icons(void)
{
    ...
    eos_bg_indicator_entry_t *entry = _s_list;
    while (entry)
    {
        if (entry->running)
        {
            ...
            lv_obj_set_size(wrap, EOS_BG_INDICATOR_ICON_SIZE, EOS_BG_INDICATOR_ICON_SIZE);
            lv_obj_set_style_radius(wrap, EOS_BG_INDICATOR_ICON_SIZE / 2, 0);
            ...
            lv_obj_set_size(icon, EOS_BG_INDICATOR_ICON_SIZE, EOS_BG_INDICATOR_ICON_SIZE);
            lv_obj_center(icon);

            /* Load the app icon image (icon.bin is a PNG). */
            if (entry->icon_src[0] && eos_storage_is_file(entry->icon_src))
            {
            ...
            else
            {
                ...
                lv_image_set_src(icon, EOS_IMG_APP);
            }
        }
        entry = entry->next;
    }
...
void eos_bg_indicator_init(void)
{
    ...
    lv_obj_t *layer = eos_overlay_get_indicator_layer();
    if (!layer)
    {
        EOS_LOG_E("BgIndicator init: indicator layer not created yet");
        return;
    }

    _s_icon_container = lv_obj_create(layer);
    lv_obj_remove_style_all(_s_icon_container);
    lv_obj_set_size(_s_icon_container, LV_SIZE_CONTENT, EOS_BG_INDICATOR_ICON_SIZE);
    ...
    _s_initialized = true;
    EOS_LOG_I("BgIndicator initialized");
}
                                       ...
                                       eos_activity_t *activity)
{
    ...
    if (_s_count >= EOS_BG_INDICATOR_MAX_APPS)
    {
        EOS_LOG_W("BgIndicator register: max background apps reached (%d)",
                  EOS_BG_INDICATOR_MAX_APPS);
        return EOS_FAILED;
    }

    entry = eos_malloc_zeroed(sizeof(eos_bg_indicator_entry_t));
    if (!entry)
        return EOS_FAILED;
    ...
    EOS_LOG_I("BgIndicator register: app_id=%s icon=%s (count=%u)",
...
eos_result_t eos_bg_indicator_unregister(const char *app_id)
{
    ...
    eos_bg_indicator_entry_t *prev = NULL;
    eos_bg_indicator_entry_t *entry = _s_list;
    while (entry)
    {
        if (strcmp(entry->app_id, app_id) == 0)
        {
            ...
            EOS_LOG_I("BgIndicator unregister: app_id=%s (count=%u)",
