 ...
 * display. Two modes:
 *   - COS_KB_MODE_EN: QWERTY letters + space / back / enter / mode-toggle.
 *     Letters and digits are inserted directly into the bound textarea.
 *   - COS_KB_MODE_ZH: the same letter keys accumulate a pinyin string; the
 *     candidate bar (number buttons) shows hanzi from cos_pinyin_lookup();
...
typedef enum
{
    COS_KB_MODE_EN = 0, /**< English QWERTY */
    COS_KB_MODE_ZH,     /**< Chinese pinyin IME */
} cos_keyboard_mode_t;
