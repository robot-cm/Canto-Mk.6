...
#include "lvgl.h"
#include "cos_app.h"
#include "cos_app_list.h"   /* cos_app_launch_immediately() */
#include "cos_service_storage.h" /* cos_storage_is_file() for icon.bin */
#include "cos_image_resuorces.h" /* COS_IMG_ALBUM (system icon on SD) */
#include "apps/gallery/cos_gallery.h" /* cos_gallery_enter() */
#include "apps/album/cos_album.h"     /* cos_album_enter() */
#include "apps/files/cos_files.h"     /* cos_files_enter() */
#include "ui/system/cos_round_clip.h" /* cos_round_clip() */
#include "cos_event.h"                 /* COS_EVENT_APP_INSTALLED / UNINSTALLED */
#include "ui/framework/cos_ui_framework.h" /* framework Home + views + profiles */
#include "cos_ui_anim.h"                      /* page transition animations */
#include "cos_service_config.h"               /* square-mode setting */
#include "cos_mem.h"                          /* cos_free() for config string */
...
static char s_app_icon_paths[LAUNCHER_APP_MAX][COS_FS_PATH_MAX];
...
static cos_framework_home_t *s_home = NULL;
static lv_timer_t *s_step_timer = NULL;
static cos_activity_t *s_launcher_activity = NULL;
/* Square-layout mode chosen at runtime (set only from _launcher_on_enter,
   ...
   stay OUT of cos_launcher_build_home / cos_launcher_rebuild — those are
...
static cos_display_profile_t s_home_profile;
...
static void _open_app(const launcher_app_def_t *app)
{
     ...
     * cos_app_launch_immediately() handles both system apps
     * (sys.settings, sys.flash_light, ...) and installed script apps. */
    cos_result_t r = cos_app_launch_immediately(app->app_id);
    if (r != COS_OK)
        COS_LOG_W("Failed to launch app '%s' (result=%d)", app->app_id ? app->app_id : "?", (int)r);
}
...
static void _build_app_defs(void)
{
    ...
    uint32_t installed = cos_app_get_installed();
    for (uint32_t i = 0; i < installed && (uint32_t)s_app_count < LAUNCHER_APP_MAX; i++)
    {
        ...
        bool is_sys = false;
        for (int s = 0; s < COS_SYS_APP_LAST; s++)
        {
            if (strcmp(id, cos_sys_app_id_list[s]) == 0)
            {
                is_sys = true;
                break;
            }
        ...
        const char *nm = cos_app_get_name(id);
        if (!nm || nm[0] == '\0')
            nm = id;
        a->name = nm;
    ...
    for (int i = 0; i < s_app_count; i++)
    {
        ...
        if (s_apps[i].app_id)
        {
            snprintf(s_app_icon_paths[i], sizeof(s_app_icon_paths[i]),
                     COS_APP_INSTALLED_DIR "%s/" COS_APP_ICON_FILE_NAME, s_apps[i].app_id);
            if (cos_storage_is_file(s_app_icon_paths[i]))
                icons[i] = s_app_icon_paths[i];
        }
        else if (s_apps[i].kind == LAUNCHER_APP_KIND_ALBUM &&
                 cos_storage_is_file(COS_IMG_ALBUM))
        {
            icons[i] = COS_IMG_ALBUM;   /* reuse the album.webp system icon */
        }
    }

    cos_framework_home_t *h = cos_framework_home_create(parent, p, names, s_app_count, icons);
    if (!h)
        return 0;
    cos_framework_home_set_on_select(h, _home_on_select, NULL);
...
static void _step_tick(lv_timer_t *t)
{
    LV_UNUSED(t);
    if (s_home)
        cos_framework_home_step(s_home, 16.0f);
}
...
void cos_launcher_rebuild(void)
{
    ...
    cos_framework_home_destroy(s_home);
    s_home = NULL;

    int n = cos_launcher_build_home(s_home_parent, &s_home_profile, &s_home);
    COS_LOG_I("Launcher: rebuild Apps grid (install/uninstall event), apps=%d", n);
    if (s_home)
    {
        cos_framework_home_set_on_select(s_home, _home_on_select, NULL);
        ...
        if (s_square_bubble && !s_home->is_circle)
            cos_framework_home_use_bubble(s_home);
...
static void _on_app_changed_cb(cos_event_t *e)
{
    LV_UNUSED(e);
...
static void _launcher_close_anim_cb(lv_anim_timeline_t *at,
                                    cos_activity_t *from, cos_activity_t *to)
{
    LV_UNUSED(to);
    lv_obj_t *view = cos_activity_get_view(from);
    if (!view) return;
    cos_anim_add_fade_scale(at, view, 255, 0, 256, 205, 0, 250);
}
...
static void _register_routes_once(void)
{
    if (s_routes_registered)
        return;
    cos_activity_register_anim_route(COS_ACTIVITY_TYPE_APP_LIST,
                                     COS_ACTIVITY_TYPE_WATCHFACE,
                                     _launcher_close_anim_cb);
    s_routes_registered = true;
}

static void _launcher_on_enter(cos_activity_t *activity)
{
    ...
    int n = cos_launcher_build_home(view, &p, &s_home);
    COS_LOG_I("Launcher apps: %d (Gallery + Files (local) + %d plugin(s))",
              n, (n > 2 ? n - 2 : 0));
    ...
    /* Square/rectangle screens honor the user's grid/bubble setting. Round
       ...
       initialized here (this runs after cos_init at runtime; the headless
       tests never enter this path), so cos_config_get_string is safe. Cache
       the decision in s_square_bubble so the rebuild path can re-apply it. */
    bool want_bubble = false;
    if (p.shape != COS_DISPLAY_SHAPE_CIRCLE)
    {
        char *mode = cos_config_get_string(COS_CONFIG_KEY_LAUNCHER_SQUARE_MODE, "grid");
    ...
    cos_event_subscribe(COS_EVENT_APP_INSTALLED, _on_app_changed_cb, NULL);
    cos_event_subscribe(COS_EVENT_APP_UNINSTALLED, _on_app_changed_cb, NULL);

    COS_LOG_I("Launcher shown");
}

static void _launcher_on_destroy(cos_activity_t *activity)
{
    LV_UNUSED(activity);
    if (s_step_timer)  { lv_timer_delete(s_step_timer);  s_step_timer = NULL; }
    cos_event_unsubscribe(COS_EVENT_APP_INSTALLED, _on_app_changed_cb);
    cos_event_unsubscribe(COS_EVENT_APP_UNINSTALLED, _on_app_changed_cb);
    if (s_home) { cos_framework_home_destroy(s_home); s_home = NULL; }
    s_launcher_activity = NULL;
}

static const cos_activity_lifecycle_t s_launcher_lc = {
 ...
 *        implementation). The dispatcher in cos_launcher.c routes here when
 *        launcher_mode == "v1".
 */
void cos_launcher_v1_enter(void)
{
    ...
    cos_activity_t *a = cos_activity_create(&s_launcher_lc);
    if (!a)
    {
        COS_LOG_E("Failed to create launcher activity");
        return;
    }
