...
{
    if (m->count >= COS_APP_MAX) return -1;
    int slot = m->count;
    strncpy(m->apps[slot].id, id, COS_APP_ID_LEN - 1);
    m->apps[slot].id[COS_APP_ID_LEN - 1] = '\0';
    strncpy(m->apps[slot].name, name, COS_APP_NAME_LEN - 1);
    m->apps[slot].name[COS_APP_NAME_LEN - 1] = '\0';
...
int cos_app_manager_count(const cos_app_manager_t *m) { return m->count; }
