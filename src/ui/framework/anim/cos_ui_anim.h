 ...
 *
 * NOTE: named eos_ui_anim (not eos_anim) to avoid colliding with the existing
 * src/ui/widgets/anim/eos_anim.h once both directories sit on the include path.
 *
 ...
 */
#ifndef EOS_UI_ANIM_H
#define EOS_UI_ANIM_H
...
float eos_ease_linear(float t);
float eos_ease_in_out_cubic(float t);
float eos_ease_out_cubic(float t);
float eos_ease_out_back(float t);     /**< overshoot, for a springy feel */
float eos_ease_out_bounce(float t);   /**< real bounce settle           */
