...
#define EOS_LAYOUT_CARD_DP 48.0f

static void _item_click_cb(lv_event_t *e);

eos_layout_view_t *eos_layout_view_create(lv_obj_t *parent,
                                          const eos_display_profile_t *p,
                                          const char **names, int n,
                                          const char **icon_paths)
{
    int count = n;
    eos_layout_view_t *v = (eos_layout_view_t *)eos_malloc(sizeof(*v));
    if (!v) return NULL;
    memset(v, 0, sizeof(*v));
    v->profile = p;
    v->count = count;
    v->card_px = EOS_LAYOUT_CARD_DP * p->dp_scale;

    v->widgets = (eos_widget_t *)eos_malloc(sizeof(eos_widget_t) * (count > 0 ? count : 1));
    v->items   = (eos_layout_view_item_t *)eos_malloc(
                     sizeof(eos_layout_view_item_t) * (count > 0 ? count : 1));
    if (!v->widgets || !v->items) {
        eos_free(v->widgets); eos_free(v->items); eos_free(v);
        return NULL;
    }

    v->mgr = eos_layout_manager_create(p);
    ...
    for (int i = 0; i < count; i++) {
        ...
        v->widgets[i].kind = EOS_WIDGET_ICON;
        v->widgets[i].w_dp = EOS_LAYOUT_CARD_DP;
        v->widgets[i].h_dp = EOS_LAYOUT_CARD_DP;
    }
    if (v->mgr) v->mgr->layout(v->mgr, p, v->widgets, count);

    for (int i = 0; i < count; i++) {
        eos_widget_t *w = &v->widgets[i];
...
void eos_layout_view_destroy(eos_layout_view_t *v)
{
    if (!v) return;
...
void eos_layout_view_set_on_select(eos_layout_view_t *v,
...
static void _item_click_cb(lv_event_t *e)
{
    eos_layout_view_t *v = (eos_layout_view_t *)lv_event_get_user_data(e);
    if (!v) return;
...
int eos_layout_view_count(const eos_layout_view_t *v)
{
    return v ? v->count : 0;
}

lv_obj_t *eos_layout_view_card_at(const eos_layout_view_t *v, int i)
{
