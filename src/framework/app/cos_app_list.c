...
#include "cos_test.h"
#endif
/* When the simulator builds the adaptive framework Launcher (COS_USE_CUSTOM_
...
const char *cos_sys_app_id_list[COS_SYS_APP_LAST] = {"sys.settings",
                                                     "sys.flash_light",
#if COS_ENABLE_TEST_APP
                                                     "sys.test"
#endif
...
const char *cos_sys_app_icon_list[COS_SYS_APP_LAST] = {COS_IMG_SETTINGS,
                                                       COS_IMG_FLASH_LIGHT,
#if COS_ENABLE_TEST_APP
                                                       COS_IMG_APP
#endif
...
const cos_sys_app_entry_t cos_sys_app_entry_list[COS_SYS_APP_LAST] = {cos_settings_enter,
                                                                      cos_flash_light_enter,
#if COS_ENABLE_TEST_APP
                                                                      cos_test_start
#endif
};
...
const char *cos_native_app_id_list[COS_NATIVE_APP_LAST] = {
    "com.cantomk6.album",
...
const char *cos_native_app_icon_list[COS_NATIVE_APP_LAST] = {
    COS_IMG_ALBUM,
    COS_IMG_TEXTHUB,
    COS_IMG_DICTIONARY,
#if defined(CONFIG_USB_MSC_APP_ENABLE) && CONFIG_USB_MSC_APP_ENABLE
...
const cos_sys_app_entry_t cos_native_app_entry_list[COS_NATIVE_APP_LAST] = {
    cos_album_enter,
    cos_texthub_enter,
    cos_dictionary_enter,
#if defined(CONFIG_USB_MSC_APP_ENABLE) && CONFIG_USB_MSC_APP_ENABLE
    cos_usb_msc_enter,
#endif
#if defined(CONFIG_USB_UAC_APP_ENABLE) && CONFIG_USB_UAC_APP_ENABLE
...
#endif
#if defined(CONFIG_PROFMONITOR_APP_ENABLE) && CONFIG_PROFMONITOR_APP_ENABLE
    cos_profmonitor_enter,
#endif
    cos_eqsolver_enter,
};

static void _app_list_on_resueme(cos_activity_t *a);

static void _app_on_enter(cos_activity_t *a);
static cos_activity_lifecycle_t app_list_lifecycle = {
    .on_enter = NULL,
    .on_destroy = NULL,
    .on_pause = NULL,
...
static void _app_on_destroy(cos_activity_t *a);
static void _app_on_pause(cos_activity_t *a);
static void _app_on_resume(cos_activity_t *a);

static cos_activity_lifecycle_t app_lifecycle = {
...
/* Pager arrow buttons (RemixIcon glyphs, see cos_icon.h / RemixIcon.c). */
...
static cos_card_pager_t *_app_list_pager = NULL;
...
static void _app_uninstalled_cb(cos_event_t *e);
static void _container_delete_cb(lv_event_t *e);
static void _app_list_refresh(lv_obj_t *bubble_grid);
static void _app_list_open_app_anim_cb(lv_anim_timeline_t *at, cos_activity_t *from, cos_activity_t *to);
static void _app_list_close_app_anim_cb(lv_anim_timeline_t *at, cos_activity_t *from, cos_activity_t *to);
static void _register_anim_routes_once(void);
static const char *_app_list_get_launch_app_id(cos_activity_t *activity);
static void _app_list_set_last_launch_app_id(const char *app_id);
static lv_obj_t *_app_list_get_bubble_grid(cos_activity_t *activity);
...
static cos_result_t _app_list_build_script_pkg(const char *app_id, script_pkg_t *pkg);
static cos_result_t _app_list_launch_script_app(const char *app_id);
...
static void _app_on_destroy(cos_activity_t *a)
{
    app_launch_ctx_t *ctx = cos_activity_get_user_data(a);

    /* If this was a background app, drop its indicator icon first. */
    if (ctx && ctx->background)
    {
        cos_bg_indicator_unregister(ctx->app_id);
    }
    ...
    if (ctx)
    {
        cos_pkg_free(&ctx->pkg);
        cos_free(ctx->app_id);
        cos_free(ctx);
        cos_activity_set_user_data(a, NULL);
    }
}

/* JS 可写 cos.config "app.background"（bool）声明"是否应作为后台运行"。
...
static bool _js_app_wants_background(const char *app_id)
{
    ...
    snprintf(path, sizeof(path), COS_APP_DATA_DIR "%s/config.json", app_id);
...
static void _app_on_pause(cos_activity_t *a)
{
    ...
    COS_LOG_I("COS-DBG app_on_pause: %s background-flag=%d", ctx->app_id, _wants);
    if (_wants)
    {
        cos_bg_indicator_register(ctx->app_id, ctx->icon_src, a);
    }
}
...
static void _app_on_enter(cos_activity_t *a)
{
    ...
    if (ret != COS_OK)
    {
        ...
        if (ret != COS_ERR_TIMEOUT)
        {
            cos_app_handle_script_error(error_type, ret, ctx->app_id, NULL);
        }
        COS_LOG_E("Application encounter a fatal error");
    }
}
...
static int32_t _app_list_find_sys_app(const char *app_id)
{
    ...
    for (int32_t i = 0; i < COS_SYS_APP_LAST; i++)
    {
        if (strcmp(app_id, cos_sys_app_id_list[i]) == 0)
        {
            return i;
        }
    }
...
static int32_t _app_list_find_native_app(const char *app_id)
{
    ...
    for (int32_t i = 0; i < COS_NATIVE_APP_LAST; i++)
    {
        if (strcmp(app_id, cos_native_app_id_list[i]) == 0)
        {
            return i;
        }
    }
...
{
    snprintf(icon_path, icon_path_size, COS_APP_INSTALLED_DIR "%s/" COS_APP_ICON_FILE_NAME, app_id);
...
#if defined(CONFIG_USB_MSC_APP_ENABLE) && CONFIG_USB_MSC_APP_ENABLE
    /* USB MSC:图标编译进 Flash(cos_icon_usb_msc),不依赖 SD 上的 .bin。
     * 直接返回 lv_image_dsc_t 指针,lv_image_set_src() 原生支持。 */
    if (native_index == COS_NATIVE_APP_USB_MSC)
    {
        return &cos_icon_usb_msc;
    }
#endif
#if defined(CONFIG_USB_UAC_APP_ENABLE) && CONFIG_USB_UAC_APP_ENABLE
    /* Spotify:图标编译进 Flash(cos_icon_spotify),不依赖 SD 上的 .bin。 */
    if (native_index == COS_NATIVE_APP_SPOTIFY)
    {
        return &cos_icon_spotify;
    }
#endif
    if (native_index >= 0 && cos_native_app_icon_list[native_index] &&
        cos_storage_is_file(cos_native_app_icon_list[native_index]))
    {
        snprintf(icon_path, icon_path_size, "%s", cos_native_app_icon_list[native_index]);
        return icon_path;
    }

    snprintf(icon_path, icon_path_size, "%s", COS_IMG_APP);
    return icon_path;
}
...
static cos_result_t _app_list_build_script_pkg(const char *app_id, script_pkg_t *pkg)
{
    ...
    if (!pkg->script_str)
    {
        COS_LOG_E("Failed to read script: %s", script_path);
        cos_pkg_free(pkg);
        return COS_FAILED;
    }
...
{
    script_pkg_t pkg = {0};
    if (_app_list_build_script_pkg(app_id, &pkg) != COS_OK)
    {
        return COS_FAILED;
    }

    app_launch_ctx_t *ctx = cos_malloc_zeroed(sizeof(app_launch_ctx_t));
    if (!ctx)
    {
        COS_LOG_E("Failed to allocate app launch context");
        cos_pkg_free(&pkg);
        return COS_FAILED;
    }

    ctx->pkg = pkg;
    ctx->app_id = cos_strdup(app_id);
    if (!ctx->app_id)
    {
        COS_LOG_E("Failed to copy app id");
        cos_pkg_free(&ctx->pkg);
        cos_free(ctx);
        return COS_FAILED;
    }
    ...
    snprintf(ctx->icon_src, sizeof(ctx->icon_src),
             COS_APP_INSTALLED_DIR "%s/" COS_APP_ICON_FILE_NAME, app_id);

    cos_activity_t *a = cos_activity_create(&app_lifecycle);
    if (!a)
    {
    ...
    lv_obj_t *app_view = cos_activity_get_view(a);
    lv_obj_set_size(app_view, COS_DISPLAY_WIDTH, COS_DISPLAY_HEIGHT);
    cos_activity_set_type(a, COS_ACTIVITY_TYPE_APP);
    cos_activity_set_user_data(a, ctx);
    ...
    cos_activity_set_app_header_visible(a, false);
    ...
    if (ctx->background)
        cos_activity_set_keep_alive_on_back(a, true);

    COS_LOG_D("view_size: %d, %d", lv_obj_get_width(app_view), lv_obj_get_height(app_view));
 ...
 * @return cos_result_t COS_OK success, COS_FAILED failed or not in app
 * @note If not in app context, returns COS_FAILED; otherwise clears the activity stack
 */
static cos_result_t _app_list_pop_to_app_list(void)
{
    ...
    cos_activity_type_t current_type = cos_activity_get_type(current);

    // If already at app list, no need to pop
    if (current_type == COS_ACTIVITY_TYPE_APP_LIST)
    {
        return COS_OK;
    }
...
cos_result_t cos_app_launch_immediately(const char *app_id)
{
    if (!(app_id && app_id[0]))
    {
        COS_LOG_E("Invalid app id");
        return COS_FAILED;
    }

    if (cos_activity_is_transition_in_progress())
    {
        COS_LOG_W("Cannot launch app while activity transition is in progress");
        return COS_FAILED;
    }
    ...
    if (sys_app_index >= 0)
    {
        if (cos_sys_app_entry_list[sys_app_index])
        {
            cos_sys_app_entry_list[sys_app_index]();
            return COS_OK;
        }
        return COS_FAILED;
    }
    ...
    {
        if (cos_native_app_entry_list[native_app_index])
        {
            cos_native_app_entry_list[native_app_index]();
            return COS_OK;
        }
        return COS_FAILED;
    }

    if (!cos_app_list_contains(app_id))
    {
        COS_LOG_E("App not found: %s", app_id);
        return COS_FAILED;
    }
    ...
    cos_activity_type_t current_type = cos_activity_get_type(cos_activity_get_current());
    if (current_type == COS_ACTIVITY_TYPE_APP)
    {
        COS_LOG_I("Returning to app list before launching new app");
...
static const char *_app_list_get_launch_app_id(cos_activity_t *activity)
{
    app_launch_ctx_t *ctx = cos_activity_get_user_data(activity);
    if (ctx && ctx->app_id)
    {
        return ctx->app_id;
    }
...
static lv_obj_t *_app_list_get_bubble_grid(cos_activity_t *activity)
{
    ...
    lv_obj_t *bubble_grid = (lv_obj_t *)cos_activity_get_user_data(activity);
    if (bubble_grid)
    {
        return bubble_grid;
    }
...
static bool _app_list_calc_focus_pivot_by_global_center(lv_obj_t *obj, int32_t *pivot_x, int32_t *pivot_y)
{
    ...
    if (max_x <= 0)
        max_x = COS_DISPLAY_WIDTH;
    if (max_y <= 0)
        max_y = COS_DISPLAY_HEIGHT;
...
static lv_obj_t *_app_list_create_icon_clone(lv_obj_t *focus_icon)
{
    ...
    lv_obj_t *icon_clone = lv_obj_create(cos_overlay_get_snapshot_layer());
                                           ...
                                           bool opening)
{
    ...
    lv_obj_t *list_view = opening ? cos_activity_get_view(from) : cos_activity_get_view(to);
    ...
    {
        app_snapshot = cos_activity_take_snapshot(from, include_header_in_snapshot);
        if (!app_snapshot)
        {
            return;
        }
    ...
    cos_activity_t *list_activity = opening ? from : to;
    if (list_activity)
    {
        list_snapshot = cos_activity_take_snapshot(list_activity, false);
    }
    ...
    if (opening)
    {
        app_snapshot = cos_activity_take_snapshot(to, include_header_in_snapshot);
        if (!app_snapshot)
        {
            if (icon_clone)
            {
    ...
    {
        int32_t view_center_x = COS_DISPLAY_WIDTH / 2;
        int32_t view_center_y = COS_DISPLAY_HEIGHT / 2;
...
static void _app_list_on_resueme(cos_activity_t *a)
{
    ...
    COS_CHECK_PTR_RETURN(bubble_grid);
...
{
    _app_grid_icon_ctx_t *ctx = (_app_grid_icon_ctx_t *)lv_event_get_user_data(e);
    COS_CHECK_PTR_RETURN(ctx && ctx->app_id);
    ...
    if (cos_app_launch_immediately(ctx->app_id) != COS_OK)
    {
        COS_LOG_E("Launch app failed: %s", ctx->app_id);
    }
}
...
static void _register_anim_routes_once(void)
{
    ...
    cos_activity_register_anim_route(COS_ACTIVITY_TYPE_APP_LIST, COS_ACTIVITY_TYPE_APP, _app_list_open_app_anim_cb);
    cos_activity_register_anim_route(COS_ACTIVITY_TYPE_APP, COS_ACTIVITY_TYPE_APP_LIST, _app_list_close_app_anim_cb);
 ...
 */
static void _app_list_append_icon(cos_card_pager_t *cp,
                                  lv_obj_t **cur_page,
                                  uint32_t *icon_index,
                                  const void *icon_src,
                                  const char *app_id)
{
    ...
    {
        *cur_page = cos_card_pager_create_page(cp);
        if (*cur_page)
        {
            lv_obj_set_style_bg_color(*cur_page, COS_COLOR_BLACK, 0);
    ...
    int32_t cx = COS_DISPLAY_WIDTH / 2
                 + col * _APP_GRID_COL_GAP
                 - (row_icons * _APP_GRID_COL_GAP / 2 - _APP_GRID_COL_GAP / 2);
    int32_t cy = (COS_DISPLAY_HEIGHT - (2 * _APP_GRID_ROW_GAP)) / 2 + row * _APP_GRID_ROW_GAP;
    ...
    _app_grid_icon_ctx_t *ctx = (_app_grid_icon_ctx_t *)cos_malloc(sizeof(_app_grid_icon_ctx_t));
    if (!ctx)
    {
        return;
    }
...
{
    cos_card_pager_t *cp = _app_list_pager;
    if (!cp)
    {
        return;
    }
...
static void _app_list_page_changed_cb(cos_card_pager_t *cp, uint8_t page_index, void *user_data)
{
    (void)cp;
    (void)page_index;
    (void)user_data;
...
static void _app_arrow_clicked_cb(lv_event_t *e)
{
    ...
    {
        cos_card_pager_move_page(cp, cp->current_page_index + 1);
    }
...
static lv_obj_t *_app_arrow_create(lv_obj_t *parent, bool next)
{
    ...
    lv_obj_set_style_text_font(label, &COS_FONT_ICON, 0);
...
static void _app_list_refresh(lv_obj_t *bubble_grid)
{
    ...
    {
        cos_card_pager_remove_page(cp, cp->page_count - 1);
    }
    ...
    cJSON *app_order = cos_config_get_json(COS_CONFIG_KEY_APP_ORDER_ARRAY);
    ...
    if (app_order && cJSON_IsArray(app_order))
    {
        ...
        cJSON_ArrayForEach(item, app_order)
        {
            ...
            {
                if (strcmp(order_id, cos_sys_app_id_list[si]) == 0)
                {
                    is_sys = true;
                    break;
                }
            ...
            const char *app_id = cos_app_list_get_existing_id(order_id);
            if (!app_id)
            {
                continue;
            }
    ...
    else
    {
        ...
        {
            const char *app_id = cos_app_list_get_id(i);
            if (!app_id)
            {
                continue;
            }
...
static void _app_installed_cb(cos_event_t *e)
{
    lv_obj_t *bubble_grid = cos_event_get_obj(e);
    COS_CHECK_PTR_RETURN(bubble_grid);
    _app_list_refresh(bubble_grid);
}
...
static void _container_delete_cb(lv_event_t *e)
{
    ...
    COS_CHECK_PTR_RETURN(bubble_grid);
    cos_event_unsubscribe_with_obj(COS_EVENT_APP_INSTALLED, _app_installed_cb, bubble_grid);
    cos_event_unsubscribe_with_obj(COS_EVENT_APP_UNINSTALLED, _app_uninstalled_cb, bubble_grid);
 ...
 */
static bool _app_list_on_swipe_back(cos_activity_t *activity, lv_dir_t dir)
{
    if (dir != LV_DIR_RIGHT)
    {
        return false;
...
void cos_app_list_enter(void)
{
    ...
    cos_activity_t *a = cos_activity_create(&app_list_lifecycle);
    if (!a)
    {
        COS_LOG_E("Failed to create activity");
        return;
    }
    cos_activity_set_type(a, COS_ACTIVITY_TYPE_APP_LIST);
    ...
    lv_obj_set_style_bg_color(view, COS_COLOR_BLACK, 0);
    ...
    cos_card_pager_t *pager = cos_card_pager_create(view, COS_CARD_PAGER_DIR_HOR);
    if (!pager)
    {
        COS_LOG_E("Failed to create card pager");
        cos_activity_back();
        return;
    }
    lv_obj_t *bubble_grid = pager->container;
    lv_obj_set_size(bubble_grid, COS_DISPLAY_WIDTH, COS_DISPLAY_HEIGHT);
    lv_obj_center(bubble_grid);
    _app_list_pager = pager;
    cos_activity_set_user_data(a, bubble_grid);
    ...
    cos_card_pager_set_page_changed_cb(pager, _app_list_page_changed_cb, NULL);
    _app_list_update_arrows();
    ...
    cos_activity_set_swipe_back_handler(a, _app_list_on_swipe_back);

    // Set callback
    lv_obj_add_event_cb(bubble_grid, _container_delete_cb, LV_EVENT_DELETE, NULL);
    cos_event_subscribe_ex(COS_EVENT_APP_INSTALLED, _app_installed_cb, NULL, bubble_grid);
    cos_event_subscribe_ex(COS_EVENT_APP_UNINSTALLED, _app_uninstalled_cb, NULL, bubble_grid);
