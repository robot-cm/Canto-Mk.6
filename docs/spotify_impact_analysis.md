...
| 项目 | 现状 | 影响 |
|---|---|---|
| `resources/images/icon/cos_icon_spotify.c` | **不存在** | 需新增（Batch 1 占位，Batch 3 正式图） |
| `resources/images/icon/cos_icon_earphone_err.c` | **不存在** | 需新增（Batch 1 必需，失败页依赖） |
...
| `cos_app_list.c` 原生 App 注册表 | 已有稳定机制 | 直接复用，只加 1 个枚举 + 3 处数组元素 |
...
> **建议新增独立宏 `COS_USB_UAC_APP_ENABLE`**（默认 1），理由见 §6。
...
| 文件 | 作用 | 批次 |
|---|---|---|
| `src/apps/spotify/cos_spotify.h` | App 对外入口 `cos_spotify_enter()` | B1 |
| `src/apps/spotify/cos_spotify.c` | App 主体：状态机 / UI / 手势 / 退出恢复 | B1–B4 |
| `src/apps/spotify/cos_spotify_board.h` | 板级能力声明（USB 占用探测 / 电源保持） | B1 |
| `src/apps/spotify/cos_spotify_uac.h` | UAC 层错误码 + 对外 API | B2 |
| `src/apps/spotify/cos_spotify_uac.c` | TinyUSB **Host** UAC 初始化 / 枚举 / 等时端点 / 去初始化 | B2 |
| `src/apps/spotify/cos_spotify_audio.h` | 解码器抽象（MP3/WAV → PCM） | B3 |
| `src/apps/spotify/cos_spotify_audio.c` | WAV 解析 + MP3 接入，PCM 环形缓冲 | B3 |
| `src/apps/spotify/cos_spotify_mp3.c` / `.h` | 内嵌轻量 MP3 解码器（minimp3 单文件） | B3 |
| `src/apps/spotify/cos_spotify_lrc.c` / `.h` | .lrc 解析 + 时间戳检索 | B3 |
| `src/apps/spotify/cos_spotify_tree.c` / `.h` | SD 懒加载文件树（参考 Texthub FM） | B3 |
| `src/apps/spotify/cos_spotify_arc.h` / `.c` | 音量弧 / 速度弧控件（120°，长按 1s/3s） | B4 |
| `resources/images/icon/cos_icon_spotify.c` | App 图标（36×36 ARGB8888） | B1 占位 |
| `resources/images/icon/cos_icon_earphone_err.c` | 失败页图标（94×94 ARGB8888） | B1 |

### 1.2 修改文件（仅 3 个，改动点全部用宏包裹）

| 文件 | 改动类型 | 具体改动 | 风险 |
|---|---|---|---|
| `src/framework/app/cos_app_list.h` | **修改** | `COS_NATIVE_APP_*` 枚举新增 `COS_NATIVE_APP_SPOTIFY` | 极低（仅枚举追加） |
| `src/framework/app/cos_app_list.c` | **修改** | 3 处数组追加 1 个元素 + `#include` + `extern` 图标 | 低（纯追加，不进刷新循环） |
| `port/esp32s3/main/CMakeLists.txt` | **修改** | 新增 `COS_USB_UAC_APP_ENABLE` 分支，加图标源 + `-u` 链接选项 | 低（新分支，不改原分支） |
...
- `src/services/storage/cos_service_storage.h`（`cos_storage_dir_open/read`、`cos_storage_file_open_read`）
- `src/ui/system/cos_round_clip.h`（圆屏裁剪）
- `src/ui/font/cos_font.h`（`cos_label_set_font_size`，中文歌词走 jbm_13→han_sans_13 链）
- `src/ui/widgets/image/cos_image_resuorces.h`
- `src/framework/activity/cos_activity.h`（生命周期 / swipe_back）
- `src/ui/symbol/cos_icon.h`（RemixIcon：播放 / 暂停 / 上下曲）
...
```text
1. cos_storage_is_dir("/sdcard/spotify")  → 无 SD / 无目录 → errcode 0x01
...
```text
 ...
 8. 销毁 LVGL 对象       : 删除本 App 全部 screen/obj（框架 cos_activity 负责 view 回收）
 9. 释放本 App 堆内存    : cos_free 全部分配（目录节点 / 歌词 / 缓冲）
10. 返回 App 启动器      : cos_activity_back()（框架既有"切换 App"机制）
```
...
| SD SPI（SPI3，与 LCD 共享） | ❌ 无 | 复用 IDF `spi_bus_lock`；本 App 所有 SD I/O 经 `cos_storage_*` 封装 |
...
- 优先级与 USB MSC 的 `cos_tusb`（prio 5）保持一致，已验证不干扰系统。
...
set(COS_USB_UAC_APP_ENABLE 1 CACHE STRING "Enable USB UAC audio Spotify App (1=on, 0=off)")
...
- `cos_app_list.h` / `cos_app_list.c` 的枚举与数组元素同样包裹；
...
现有原生 C App 注册机制（`cos_app_list.c` 已验证）：
...
```c
// cos_app_list.c —— 三个数组各追加一项
const char *cos_native_app_id_list[]   = { ..., "com.cantomk6.spotify" };
const char *cos_native_app_icon_list[] = { ..., COS_IMG_SPOTIFY };
const cos_sys_app_entry_t cos_native_app_entry_list[] = { ..., cos_spotify_enter };
```

- 图标：与 USB MSC 同法，`extern const lv_image_dsc_t cos_icon_spotify;` 编译进 Flash，
  在 `_app_list_resolve_icon()` 内加一个 `if (native_index == COS_NATIVE_APP_SPOTIFY) return &cos_icon_spotify;`。
- App 页点击 → `cos_app_launch_immediately("com.cantomk6.spotify")` → 命中 `cos_native_app_entry_list` → `cos_spotify_enter()`。
...
| 6 | `cos_icon_spotify.c` / `cos_icon_earphone_err.c` 不存在 | 低 | Batch 1 生成占位图（1×1 或纯色）保证可编译 |
...
- `COS_USB_UAC_APP_ENABLE=1`：App 页出现 Spotify 图标，点击进入后显示探测中/失败页，可滑动退出。
- `COS_USB_UAC_APP_ENABLE=0`：Spotify 完全消失，`idf.py size` 与改动前一致。
