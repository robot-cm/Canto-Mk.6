...
- **网络服务 (Network)**
  - Wi-Fi：扫描 / 连接 / 历史记忆 / 自动重连（`eos_net_wifi`）
  - 蓝牙 BLE（`eos_net_bt`）
...
- **触觉反馈 (Haptic)**：`eos_service_haptic`
- **音频播放**：`eos_audio_player`
- **插件管理 (Plugin Manager)**：扫描/安装/卸载 SD 卡 `.eapk` App（`eos_plugin_manager`）
- **输入法 (IME/Pinyin)**：`eos_pinyin` 中文拼音输入
- **包管理 (Package Manager)**：`.eapk` / `.ewpk` 安装卸载（`eos_pkg_mgr`）
- **Launcher**：圆形弧形 App 网格（`eos_launcher_v1` + `eos_arclist`）
- **Watchface**：内置表盘 + JS 表盘支持（`eos_watchface_builtin` / `eos_watchface_js`）
...
3. SD 卡示例文件见 `examples-sdcard-files.7z`, `README` 参考 `ElenixOS-fork/examples-sdcard`
4. 从源码构建：

```
    cd /.../ElenixOS-fork/port/esp32s3 && idf.py -p /dev/ttyACM0 build flash monitor
```
...
python3.11 scripts/eos_pkg_builder.py apps/breach eapk-target/breach.eapk --type app 

python3.11 scripts/eos_pkg_builder.py apps/calculator eapk-target/calculator.eapk --type app 

python3.11 scripts/eos_pkg_builder.py apps/calendar eapk-target/calendar.eapk --type app 

python3.11 scripts/eos_pkg_builder.py apps/globaltime eapk-target/globaltime.eapk --type app 

python3.11 scripts/eos_pkg_builder.py apps/stopwatch eapk-target/stopwatch.eapk --type app 

python3.11 scripts/eos_pkg_builder.py apps/timer eapk-target/timer.eapk --type app 
```
