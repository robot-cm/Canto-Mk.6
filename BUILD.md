...
| **Python 3** | 打包 `.eapk`（`scripts/eos_pkg_builder.py`）与 jerryscript 配置阶段 |
...
脚本自动完成：重新配置 → 编译 `elenixos_sim` → 打包全部 `.eapk` → 21 步 headless 自检（压力/Shell/输入法/各 app 探针），全绿输出 `[OK]` 结论。
...
| 产物 | 位置 | 说明 |
|------|------|------|
| 模拟器主程序 | `simulator/build/elenixos_sim.exe` | SDL2.dll 已自动拷到同目录 |
| 模拟文件系统 | `simulator/build/fs/` | 资源/字体/样例/eapk 均在此 staging |
| 各 App 包 | `simulator/build/fs/.sys/app/com.elenix.*.eapk` | POST_BUILD 自动打包 |
...
cd simulator/build && ./elenixos_sim.exe

# 无头（headless，供 CI/脚本用）
cd simulator/build && SDL_VIDEODRIVER=dummy ./elenixos_sim.exe --<probe>
```
...
> **注意**：运行实例读的是 `simulator/build/fs/.sys/app/apps/com.elenix.<id>/`（boot 时从 eapk 解包）。若改了 JS 但运行仍旧，删除该 app 的安装子目录（约 2 个文件）再启动，boot 会重新安装新包。改图标可跑 `scripts/icon/sync_app_icons.py` 把 `icon/*.webp` 同步为 48px `icon.bin` 并直覆运行实例。
...
- **`eos`/SNI 相关**：JS 运行时按 `eos.*` 桥接（`src/script_engine/sni/`），勿依赖未注册 API（如 `clearFlag`），事件回调内 `eos` 全局不可解析，需闭包捕获。

## 8. 其它构建目标（简介）

- **MCU（ESP-IDF）**：`EOS_PLATFORM` 非 `Native`，`fs/` 位于工程根（`EOS_SYS_ROOT_DIR` 指向源码 `fs/`），走 ESP-IDF 工具链，非本模拟器流程。
- **WASM**：`EOS_IS_WASM=1`，`EOS_SYS_ROOT_DIR="/"`，见根 `CMakeLists.txt` 分支。
